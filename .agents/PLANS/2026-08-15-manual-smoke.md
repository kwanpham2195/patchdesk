...
- [x] Corrupt `review.json` (the Review record itself): FIXED `26d57e3`, same philosophy as insights. On `invalid_stored_value` the corrupt record is MOVED ASIDE (quarantine, timestamped under `workbenches/.quarantine/`) and the Review is REBUILT FRESH from the PR identity — the Review never refuses to open because of corrupt local data. Quarantine is now only a move-aside step before rebuilding; it is never a blocking outcome. Verified live on pr-571: corrupted record -> `/v1/reviews/open` 200, fresh Open record recreated (identity-derived id unchanged), old artifacts under `.quarantine/`; restored the backup afterwards, analysis back to `current` with 6 findings. Regression test in `review-workbench-controller.test.ts`.
