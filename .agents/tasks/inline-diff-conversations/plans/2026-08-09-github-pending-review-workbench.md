...
> **Executor instructions:** This plan replaces the local Review draft and its hidden bottom dock with a GitHub-native pending-review workflow. Do not implement the renderer first and fake remote state. First complete the disposable-PR API spike in Step 1, update the product decisions named in Step 2, and stop if its results invalidate the proposed adapter contract. All GitHub writes remain capability-protected, freshness-gated, exact-head checked, and explicitly user-confirmed. Do not make live GitHub writes during ordinary automated or Electron QA.
...
7. Patchdesk retains explicit refresh. A new head shows Updates available; Refresh imports the current pending-review state and marks outdated anchors. New Patchdesk writes require the refreshed revision. The maintainer may still submit or discard an existing pending review after refresh, matching GitHub’s behavior.
...
- `.agents/tasks/inline-diff-conversations/spec.md` and `tech-spec.md` exclude pending reviews and require immediate direct comments.
...
2. Load the review and its pending comments as the reviewer through the smallest candidate REST and GraphQL reads. Confirm that the query can identify `PENDING`, current author, full thread location, and the owning review without paging the whole PR.
...
6. Push a new head. Refresh the read model, inspect anchor/outdated state, attempt an additional pending inline comment, and attempt review submission.
...
**Stop condition:** do not write production contracts until the spike confirms a bounded authenticated pending-review read, a safe add-thread mutation, discard semantics, and the behavior of replies/state changes. If GitHub does not expose a needed operation, revise the product scope before implementation; do not emulate it through local draft mirroring.
...
Replace the persisted `batchContent` owner and the `ReviewBatch` projection with this state. Remove local include/exclude, local body editing, anchor repair, and successor-local-draft rotation rather than keeping a compatibility path. No migration or silent re-publication of existing local drafts is part of this change; their treatment must be explicitly approved before code that deletes persisted batch content runs.
...
Every target-specific write must first prove current PR membership and viewer ownership where GitHub requires it. Every mutation must run after `ReviewWriteGate.requireFresh()` and a final current-head read, except submitting/discarding the already-created review after an explicit refresh, per the product decision. New comment writes use exact represented coordinates and current head. Mutations return semantic receipts and typed failure variants; they never leak body text, IDs, raw GitHub output, or credentials to the renderer/logs.
...
1. load/reconcile the viewer’s pending review at Review open, explicit refresh, and recovery;
...
Retain the existing exact-head and freshness gate for all new write commands. Keep explicit Refresh as the only remote snapshot replacement. On a head change, preserve the remote pending review, render outdated anchors truthfully, and block new coordinates until refresh. Do not attempt local anchor migration or mutation replay. For `Submitting`, `Discarding`, or any uncertain outcome, lock conflicting actions; show **Check GitHub again** and **Open on GitHub**, then reconcile before another write.
...
2. Add the header button beside PR overview/Open on GitHub. Use **Start a review** when `None`; use **Finish review · N** when `Pending`. Keep it visible in Files and Insights, absent/disabled only for terminal Reviews or an outcome-unknown lock. It must communicate blocked freshness with text, not color alone.
...
4. The dialog shows final summary input, expandable pending comments with exact path/range context, Comment/Approve/Request changes, a clear submit action, and a destructive Discard review confirmation. Submission disables close/duplicate actions; unknown recovery supplies Check GitHub again and Open on GitHub.
...
- `tests/renderer/review-workbench-flow.ui.test.tsx`: Start/Finish/refresh/recovery/Analysis/Finding end-to-end projection.
...
- The new remote state cannot replace `ReviewBatch` without silently deleting persisted user drafts; obtain an explicit retain/export/discard decision before deleting, ignoring, or clearing `batchContent`.
