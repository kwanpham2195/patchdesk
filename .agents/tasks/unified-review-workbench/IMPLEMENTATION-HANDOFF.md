...
- Fresh automated evidence after the UI cleanup, Insight disposition slice, reader slice, draft-edit slice, and retained-reader action hardening: `pnpm lint`, `pnpm typecheck`, `pnpm test -- --run` (101 files/648 tests), including the authoritative draft route slice, patch-stat regressions, completion authorization, publication preview/confirmation, refresh revocation, merge-policy, body-only publication, and migration slices. `pnpm build` also passes., `pnpm build`, focused Insight/controller tests, and `git diff --check` passed. The prior browser gate remains `pnpm exec playwright test` (60 passed).
