...
- **Feedback and merge — Task 2 in progress.** General feedback, strict Finding-to-draft provenance, legacy `ReviewDraft` removal, deterministic Analysis-body rendering, core serialized draft edit commands, and deterministic Analysis draft seeding now exist. Merge/replace persistence and previews, publication authorization, receipt-backed publication, Published feedback, merge policy, migration, final UI, and end-to-end acceptance remain.
...
- Fresh automated evidence after the UI cleanup, Insight disposition slice, reader slice, draft-edit slice, and retained-reader action hardening: `pnpm lint`, `pnpm typecheck`, `pnpm test -- --run` (99 files/644 tests), including the authoritative draft route slice, patch-stat regressions, completion authorization, and publication preview/confirmation slices. Task 3 focused authorization tests pass (14 tests across domain, storage, completion service, and coordinator); publication-focused tests pass (39 tests)., `pnpm build`, focused Insight/controller tests, and `git diff --check` passed. The prior browser gate remains `pnpm exec playwright test` (60 passed).
