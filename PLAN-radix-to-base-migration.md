...
- [x] 2026-07-19: Milestone 1 complete. Migrated shared/simple wrappers: button (+ alert-dialog compat fix), badge, separator, label, checkbox (+ PointerEvent jsdom polyfill in tests/setup.ts), scroll-area, tabs, toggle, toggle-group (toggle-variants.ts deleted), button-group, item. 8 wrappers remain on Radix: dialog, alert-dialog, sheet, dropdown-menu, select, popover, tooltip, sidebar.
