...
- Added a column of the pull requests you have opened, beside the app frame and present on every screen. It lists the active workspace's visited pull requests, most recently opened first, with the one you are reading marked, and clicking a row resumes that pull request where you left it and moves it back to the top. Every row is read from what Patchdesk already stored, so the column costs no GitHub call and works with the network off. A pull request appears once you have opened it — the Pull requests table is still where you find new work — and one you opened before this release shows its number until the next time you open it, which fills in its title. The column collapses from a button in the header and stays collapsed until you open it again. #119
