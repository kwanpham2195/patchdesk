...
export function renderAnalysisReviewBody(input: {
  ...
  const callouts = [
    ...(input.result.callouts ?? []).map(
      (callout) => `- **${text(callout.title)}**: ${text(callout.detail)}`,
    ),
    ...(input.result.unresolvedItems ?? []).map(
      (item) => `- Unresolved: ${text(item)}`,
    ),
    ...input.result.assumptions.map((item) => `- Assumption: ${text(item)}`),
  ];
  if (callouts.length > 0)
    sections.push(`# Human Reviewer Callouts\n${callouts.join("\n")}`);
