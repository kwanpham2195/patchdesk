...
export function loadInboxViewPreferences(
  ...
  try {
    return parsePreferences(
      JSON.parse(window.localStorage.getItem(key(profileId)) ?? "null"),
    );
  } catch {
...
function parsePreferences(input: unknown): InboxViewPreferences {
  ...
  return {
    ...
    sort:
      value.sort === "updated" || value.sort === "repository"
        ? value.sort
        : "priority",
    selectedRepo:
      typeof value.selectedRepo === "string"
        ? value.selectedRepo.slice(0, 200)
        : "",
    ...
    ...(typeof value.selectedIdentity === "string" &&
    value.selectedIdentity.length > 0
...
function parseSavedViews(value: unknown): ReadonlyArray<SavedInboxView> {
  ...
  for (const item of value.slice(0, 20)) {
    ...
    parsed.push({
      ...
      sort:
        item.sort === "updated" || item.sort === "repository"
          ? item.sort
          : "priority",
      selectedRepo:
        typeof item.selectedRepo === "string"
          ? item.selectedRepo.slice(0, 200)
          : "",
    });
  }
  return parsed;
