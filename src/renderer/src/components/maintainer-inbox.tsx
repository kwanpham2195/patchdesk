...
export function MaintainerInbox({
  ...
  readonly onOpenReview: (row: InboxRow, initialSection?: ReviewInitialSection) => void;
  ...
  const [selectedRepo, setSelectedRepo] = useState<string>(preferences.selectedRepo);
  ...
  useEffect(() => {
    ...
    const onAction = (): void => {
      if (selected !== undefined) requestAction(selected, onOpenReview, onOpenReviewId);
    };
    ...
    <div className="min-w-0">
      ...
      <div ref={listRef} role="listbox" aria-label="Pull requests" tabIndex={0} onKeyDown={onListKeyDown} className="divide-y outline-none focus-visible:ring-2 focus-visible:ring-ring">
        {visibleRows.map((row) => {
          ...
          return <InboxRowItem key={key} row={row} selected={active} onSelect={() => selectRow(row)} onAction={() => requestAction(row, onOpenReview, onOpenReviewId)} />;
        })}
  ...
  return (
    <div className={cn("min-h-[calc(100vh-3rem)] min-w-0 bg-background min-[1280px]:grid min-[1280px]:h-full min-[1280px]:min-h-0 min-[1280px]:overflow-hidden", desktopGridColumns)}>
      <QueueRail rows={rows} view={view} savedViews={savedViews} selectedRepo={selectedRepo} open={queueOpen} onSelect={selectView} onSelectSaved={selectSavedView} onSaveCurrent={() => setSaveViewOpen(true)} onDeleteSaved={setDeleteView} onToggle={toggleQueue} />
      ...
      <aside className={cn("hidden min-w-0 overflow-hidden border-l min-[1280px]:block", !inspectorOpen && "min-[1280px]:hidden")} aria-label="Review details">
        <ScrollArea className="h-full overflow-x-hidden"><Inspector {...(selected === undefined ? {} : { row: selected })} freshness={freshness} onAction={() => selected === undefined ? undefined : requestAction(selected, onOpenReview, onOpenReviewId)} /></ScrollArea>
      </aside>
      <Sheet open={narrow && inspectorOpen && selected !== undefined} onOpenChange={(open) => { if (!open && narrow) toggleInspector(); }}>
        <SheetContent side="right" className="w-[min(24rem,calc(100vw-1rem))] p-0 min-[1280px]:hidden">
          ...
          <Inspector {...(selected === undefined ? {} : { row: selected })} freshness={freshness} onAction={() => selected === undefined ? undefined : requestAction(selected, onOpenReview, onOpenReviewId)} />
        </SheetContent>
      </Sheet>
...
function QueueRail({ rows, view, savedViews, selectedRepo, open, onSelect, onSelectSaved, onSaveCurrent, onDeleteSaved, onToggle }: { readonly rows: ReadonlyArray<InboxRow>; readonly view: InboxView; readonly savedViews: ReadonlyArray<SavedInboxView>; readonly selectedRepo: string; readonly open: boolean; readonly onSelect: (view: InboxView) => void; readonly onSelectSaved: (view: SavedInboxView) => void; readonly onSaveCurrent: () => void; readonly onDeleteSaved: (view: SavedInboxView) => void; readonly onToggle: () => void }): React.JSX.Element {
...
function InboxRowItem({ row, selected, onSelect, onAction }: { readonly row: InboxRow; readonly selected: boolean; readonly onSelect: () => void; readonly onAction: () => void }): React.JSX.Element {
  ...
  return <button id={`inbox-row-${key}`} type="button" role="option" aria-selected={selected} onClick={() => { onSelect(); onAction(); }} className={cn("block w-full content-auto border-l-2 border-transparent px-3 py-2 text-left transition-colors [contain-intrinsic-size:auto_60px] hover:bg-muted/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring", selected && "border-l-primary bg-primary/8")}>
    <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-start gap-x-3 gap-y-1 min-[1280px]:grid-cols-[minmax(0,1fr)_8rem_2.5rem]">
      ...
      <div className="col-span-2 flex min-w-0 items-center gap-1.5 text-[11px] text-muted-foreground min-[1280px]:hidden"><CheckBadge overall={row.checks.overall} /><span><Reason row={row} /></span></div>
    </div>
  </button>;
}
...
function viewCount(rows: ReadonlyArray<InboxRow>, view: InboxView, selectedRepo: string): number { return filterRows(rows, view, "", selectedRepo).length; }
...
function filterRows(rows: ReadonlyArray<InboxRow>, view: InboxView, search: string, selectedRepo: string = ""): ReadonlyArray<InboxRow> { const needle = search.trim().toLocaleLowerCase(); return rows.filter((row) => matchesView(row, view) && (needle.length === 0 || `${row.identity.owner}/${row.identity.repo} ${row.title} ${row.author} #${row.identity.number}`.toLocaleLowerCase().includes(needle)) && (selectedRepo.length === 0 || `${row.identity.owner}/${row.identity.repo}` === selectedRepo)); }
function matchesView(row: InboxRow, view: InboxView): boolean { switch (view) { case "all_open": return true; case "my_inbox": return row.categories.some((category) => category === "needs_review" || category === "updated_since_review" || category === "saved_review"); case "updated": return row.categories.includes("updated_since_review"); case "needs_review": return row.categories.includes("needs_review"); case "waiting": return row.categories.includes("waiting_for_author"); case "checks_failing": return row.categories.includes("checks_failing"); case "ready_to_merge": return row.categories.includes("ready_to_merge"); } }
...
function priority(row: InboxRow): number { if (row.categories.includes("saved_review")) return 0; if (row.categories.includes("updated_since_review")) return 1; if (row.categories.includes("needs_review")) return 2; if (row.categories.includes("waiting_for_author")) return 3; if (row.categories.includes("checks_failing")) return 4; if (row.categories.includes("ready_to_merge")) return 5; return 6; }
function requestAction(row: InboxRow, onOpenReview: (row: InboxRow, initialSection?: ReviewInitialSection) => void, onOpenReviewId: (reviewId: string) => void): void { switch (row.recommendedAction.kind) { case "run_review": onOpenReview(row); return; case "open_saved_review": case "open_merge_readiness": case "open_discussion": onOpenReviewId(row.recommendedAction.reviewId); return; } }
function actionIcon(kind: InboxRow["recommendedAction"]["kind"]): React.JSX.Element { return kind === "run_review" ? <CheckCircle2 /> : <Clock3 />; }
