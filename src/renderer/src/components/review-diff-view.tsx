...
function ReviewDiffSurface({
  ...
  // mid-paragraph of the wrong file, while the file the reader was on
  // stays meaningful.
  //
  // The obvious source for "the reader's place" is `activePathRef`, but
  // the effect just above wipes it on every `fileMode` change -- exactly
  // the moment this effect needs its *previous* value. `useLayoutEffect`
  // is required, not `useEffect`: a layout effect's cleanup runs
  // synchronously during the same commit's layout phase, strictly before
  // any passive effect (including that wipe) gets to run, so the value is
  // still intact when this effect's cleanup reads it.
  const restoreFilePathRef = useRef<string | undefined>(undefined);
  // `items`, `selectedPath`, and `onActiveFileChange` read through this
  // ref rather than closed over directly: each can change (a sidebar click
  // while still in "selected" mode, a hydration response bumping `items`)
  // without changing `codeViewKey`, and this effect must not re-run for
  // that routine churn -- doing so would treat it as a view switch and
  // re-snap the scroll position to the top of the current file. Reading
  // through the ref keeps every value fresh at both capture and restore
  // time without making any of them a dependency.
  const restoreSourceLatest = useLatestCommitted({
    items,
    selectedPath,
    onActiveFileChange,
  });
  useLayoutEffect(() => {
    ...
    const targetPath = restoreFilePathRef.current;
    restoreFilePathRef.current = undefined;
    // The capture-for-next-time cleanup below must always be registered,
    // even on a run with nothing to restore (notably the very first mount):
    // it is what lets the *following* rebuild find a target. Nesting the
    // `return` inside an `if (targetPath !== undefined)` would make that
    // first, target-less run skip registering any cleanup at all, and the
    // capture would never get a chance to arm itself for the next rebuild.
    let cancel: (() => void) | undefined;
    if (targetPath !== undefined) {
      const {
        items: currentItems,
        onActiveFileChange: currentOnActiveFileChange,
      } = restoreSourceLatest.current;
      cancel = materializeAndScrollTo({
        viewer,
        items: currentItems,
        itemId: targetPath,
        // This effect's own cleanup below cancels any pending attempt
        // before a newer target is captured, so `attempt` can never fire
        // for a target this effect has already moved past -- unlike the
        // keyboard-jump listeners further down, which stay mounted across
        // many keypresses and need a token to tell an old press from a new
        // one within one long-lived effect.
        isStale: () => false,
        buildTarget: () => ({ type: "item", id: targetPath, align: "start" }),
        onScrolled: () => {
          // Mirrors `updateActivePath`: only meaningful in "all" mode,
          // where `activePathRef` drives scroll-tracking and
          // `onActiveFileChange` syncs a file-tree highlight. "selected"
          // mode always shows exactly one file and never reads either.
          if (preferences.fileMode !== "all") return;
          activePathRef.current = targetPath;
          currentOnActiveFileChange?.(targetPath);
        },
      });
    }
    return () => {
      cancel?.();
      // Captured for the *next* rebuild. Safe to read `preferences.fileMode`
      // directly here (not through the latest-ref above): any change to it
      // changes `codeViewKey` too, so this effect always re-runs -- with a
      // fresh closure over the new `fileMode` -- in the very commit that
      // changed it; a stale read is not possible. In "all" mode the
      // reader's place is whatever file `activePathRef` is tracking; in
      // "selected" mode there is only ever one file on screen, so it's
      // that file.
      restoreFilePathRef.current =
        preferences.fileMode === "all"
          ? activePathRef.current
          : restoreSourceLatest.current.selectedPath;
    };
  }, [codeViewKey, preferences.fileMode, restoreSourceLatest, virtualized]);
