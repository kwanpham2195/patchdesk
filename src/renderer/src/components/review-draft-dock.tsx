    ...
    <section className="border-t bg-background px-4 py-3" aria-label="Review draft dock">
      <Collapsible open={open} onOpenChange={setOpen}>
        <div className="flex items-center justify-between gap-3">
          ...
          <span className="text-xs text-muted-foreground">Decision · {batch.suggestedEvent}</span>
        </div>
        <CollapsibleContent motion="disclosure" className="pt-3">
          <ReviewBatchPanel batch={batch} {...(patch === undefined ? {} : { patch })} writeBlocked={writeBlocked} actions={actions} />
        </CollapsibleContent>
      </Collapsible>
    </section>
  );
