...
export function InsightRunDialog({
  ...
  const reasoningOptions = selectedModel?.reasoning ?? ["low", "medium", "high"];
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent data-testid="insight-run-dialog">
        <DialogHeader>
          <DialogTitle>{actionLabel}</DialogTitle>
          <DialogDescription>Choose the provider, model, and reasoning effort before starting this bounded {noun} run.</DialogDescription>
        </DialogHeader>
        <div className="grid gap-4">
          <label className="grid gap-1.5 text-sm font-medium" htmlFor="insight-run-provider">
            Provider
            <select id="insight-run-provider" aria-label="Insight provider" className="h-8 rounded-lg border border-input bg-transparent px-2.5 text-sm" value={provider} onChange={(event) => onProviderChange(event.target.value as InsightProvider)}>
          ...
          {provider === "codex-cli-account" && models.length === 0 ? (
            <div className="grid gap-2 rounded-lg border border-dashed p-3 text-sm">
              ...
              <Button variant="outline" disabled={codexActivationPending} onClick={onActivateCodex}>{codexActivationPending ? "Loading Codex models…" : "Load Codex models"}</Button>
              {codexActivationError ? <p className="text-destructive">Codex models are unavailable. Check external login and the app launch PATH.</p> : null}
            </div>
          ) : null}
          <label className="grid gap-1.5 text-sm font-medium" htmlFor="insight-run-model">
            Model
            <ModelCombobox id="insight-run-model" ariaLabel="Insight model" options={models} value={model} onValueChange={onModelChange} />
          </label>
        ...
        <DialogFooter>
          ...
          <Button data-testid="insight-run-confirm" disabled={model === null || (provider === "codex-cli-account" && models.length === 0)} onClick={onConfirm}>Start run</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
