...
export function PublishedFeedbackPanel({
  ...
  return (
    <section tabIndex={-1} className="min-h-0 border-t px-4 py-2 outline-none" aria-label="Published feedback" data-review-published-feedback-state={open ? "expanded" : "collapsed"}>
      <Collapsible open={open} onOpenChange={setOpen} className="flex min-h-0 flex-col">
        <h2 className="sr-only">Published feedback</h2>
        <div className="flex shrink-0 items-center justify-between gap-2">
          <button type="button" data-published-feedback-trigger aria-expanded={open} onClick={() => setOpen((current) => !current)} className="-ml-2 inline-flex min-h-8 items-center gap-2 rounded-lg px-2 text-sm font-semibold outline-none hover:bg-muted focus-visible:ring-3 focus-visible:ring-ring/50">
            Published feedback
            <Badge variant="secondary">{count}</Badge>
            <span aria-hidden="true" className="text-xs text-muted-foreground">{open ? "Collapse" : "Expand"}</span>
          </button>
          <p className="hidden text-xs text-muted-foreground sm:block">Remote GitHub reviews and inline comments.</p>
        </div>
        <CollapsibleContent motion="disclosure" className="min-h-0 max-h-[min(25vh,16rem)] overflow-y-auto pt-3" data-published-feedback-scroll>
