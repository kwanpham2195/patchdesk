...
export function SummaryReviewDialog({
  ...
  const recovery = state === "recovery_required" && !writeAnother ? recoveryResolution ?? "check_required" : undefined;
  ...
  return <Dialog open={open} onOpenChange={(next) => { if (!locked) onOpenChange(next); }}>
    <DialogContent className="w-[calc(100vw-2rem)] max-w-xl min-w-0" aria-label="Write review summary">
      {recovery === undefined ? <DialogHeader>
      ...
      {effectiveState === "confirmed" ? (
        ...
        <div className="space-y-4 rounded-md border border-amber-500/50 bg-amber-500/10 p-4 text-amber-950 dark:text-amber-100">
          <div className="space-y-1">
            <h2 className="font-semibold">Review submission needs confirmation</h2>
            <p role="alert" className="text-sm">{recovery === "manual_resolution_required"
              ? "Patchdesk found ambiguous review evidence. Submit is paused until you resolve the outcome on GitHub."
              : "Patchdesk did not receive confirmation from GitHub. Your review may already have been published. To avoid posting a duplicate review, submission is paused until GitHub is checked."}</p>
            <p className="text-sm">Checking either confirms the review, restores a safe submit state, or identifies manual resolution.</p>
          </div>
          {localError === undefined ? null : <p role="alert" className="text-sm">{localError}</p>}
          <div className="flex flex-wrap justify-end gap-2">
            <Button variant="outline" disabled={locked} onClick={() => onOpenChange(false)}>Close</Button>
            {onOpenPullRequest === undefined ? null : <Button variant="outline" disabled={locked} onClick={onOpenPullRequest}>Open pull request on GitHub</Button>}
            <Button disabled={locked} onClick={() => void recover()}>{recovering ? "Checking GitHub…" : "Check GitHub status"}</Button>
          </div>
        </div>
      ) : (
        <div className="space-y-4">
          ...
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            ...
            <Select value={event} onValueChange={(value) => setEvent(value as Event)}>
              ...
              <SelectContent>
                ...
                <SelectItem value="APPROVE" disabled={approvalCapability === "blocked_author"}>Approve</SelectItem>
                <SelectItem value="REQUEST_CHANGES">Request changes</SelectItem>
              </SelectContent>
            </Select>
            {approvalCapability !== "blocked_author" ? null : <p className="text-sm text-muted-foreground">You can’t approve your own pull request. Choose Comment or ask another reviewer to approve it.</p>}
          </div>
          {localError === undefined && error === undefined ? null : <p role="alert" className="text-sm text-destructive">{error ?? localError}</p>}
          <div className="flex flex-wrap justify-end gap-2">
            <Button variant="outline" disabled={locked} onClick={() => onOpenChange(false)}>Close</Button>
            <Button disabled={locked || body.trim().length === 0 || (event === "APPROVE" && approvalCapability === "blocked_author")} onClick={() => void submit()}>{submitting ? "Submitting…" : "Submit review"}</Button>
          </div>
        </div>
      )}
    </DialogContent>
