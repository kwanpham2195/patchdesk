...
import { LocalPiProviderCatalog, providerCatalog, type PiProviderCatalog, type PiProviderStatus } from "./pi-provider-catalog";
...
export class LocalPiRuntimeModelCatalog implements PiRuntimeModelCatalog {
  ...
  async get(): Promise<Result<PiRuntimeModelCatalogSnapshot, PiRuntimeModelCatalogUnavailable>> {
    ...
    const preferred = preferences.defaultModel === undefined
      ? preferences.enabledModels
      : [preferences.defaultModel, ...preferences.enabledModels.filter((model) => model !== preferences.defaultModel)];
    const configuredProviders = new Set(providerCatalog.value.providers.filter((provider) => provider.configured).map((provider) => provider.id));
    const models = projectModels(preferred, configuredProviders);
    const defaultModel = canonicalModelId(preferences.defaultModel);
    return snapshot(models, defaultModel !== undefined && models.some((model) => model.id === defaultModel) ? defaultModel : undefined, providerCatalog.value);
  }
}

function snapshot(models: ReadonlyArray<PiRuntimeModel>, defaultModel: string | undefined, providers: PiProviderCatalog): Result<PiRuntimeModelCatalogSnapshot, PiRuntimeModelCatalogUnavailable> {
  return ok({
    models,
...
function readPreferences(raw: string | undefined): { enabledModels: ReadonlyArray<string>; defaultModel?: string } {
  ...
  return parsed.success
    ? { enabledModels: parsed.output.enabledModels ?? [], ...(parsed.output.defaultModel === undefined ? {} : { defaultModel: parsed.output.defaultModel }) }
    : { enabledModels: [] };
}

function projectModels(preferred: ReadonlyArray<string>, configuredProviders: ReadonlySet<string>): ReadonlyArray<PiRuntimeModel> {
  ...
  const catalog = PI_AI_CATALOG
    .filter(({ provider }) => providerCatalog().some((definition) => definition.id === provider))
  ...
  return [...eligible]
    .sort((left, right) => (preferredIds.get(left.id) ?? Number.MAX_SAFE_INTEGER) - (preferredIds.get(right.id) ?? Number.MAX_SAFE_INTEGER))
...
export function canonicalModelId(value: string | undefined): string | undefined {
  ...
  return provider.length > 0 && model.length > 0 && /^[a-z0-9][a-z0-9._:-]*$/i.test(provider) && /^(?:[^\s/]+\/)*[^\s/]+$/.test(model) && providerCatalogIds.has(provider)
    ? `${provider}/${model}`
    : undefined;
}

const providerCatalogIds = new Set([
  "amazon-bedrock", "ant-ling", "anthropic", "azure-openai-responses", "cerebras", "cloudflare-ai-gateway", "deepseek", "fireworks", "google", "google-vertex", "groq", "huggingface", "kimi-coding", "minimax", "minimax-cn", "mistral", "moonshotai", "moonshotai-cn", "nvidia", "openai", "opencode", "opencode-go", "openrouter", "together", "vercel-ai-gateway", "xai", "xiaomi", "xiaomi-token-plan-ams", "xiaomi-token-plan-cn", "xiaomi-token-plan-sgp", "zai", "zai-coding-cn",
]);
