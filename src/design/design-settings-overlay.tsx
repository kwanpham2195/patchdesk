 ...
 * shell. It always starts on General and uses only the local copy map.
 */
export function DesignSettingsOverlay({
  ...
  const [pendingCleanup, setPendingCleanup] = useState<CleanupActionKey>();
  ...
  return (
    <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
      ...
      >
        ...
        <Tabs value={section} onValueChange={(value) => {
          ...
          <ScrollArea className="max-h-[60vh] rounded-md border p-2">
            <TabsContent value="general" className="flex flex-col gap-4 p-2">
              <Card>
                <CardHeader>
            ...
            <TabsContent value="review" className="flex flex-col gap-4 p-2">
              <Card>
                <CardHeader>
            ...
            <TabsContent value="data" className="flex flex-col gap-4 p-2">
              <Card data-testid="local-review-data-card">
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="flex flex-col gap-2">
                    <Button variant="outline" onClick={() => setPendingCleanup("clear_cache")}>Clear cache</Button>
                    <Button variant="outline" data-testid="clear-local-data-button" onClick={() => setPendingCleanup("clear_local_review_data")}>Clear local review data</Button>
                  </div>
      ...
      <CleanupDialog pending={pendingCleanup} onCancel={() => setPendingCleanup(undefined)} onConfirm={() => setPendingCleanup(undefined)} />
    </Dialog>
  );
}
