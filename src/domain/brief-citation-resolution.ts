 ...
 * `brief.ts` re-exports `resolveBriefCitations`, so an existing importer of it
 * from `./brief` needs no change; `brief-flow.ts` imports it from here
 * directly, which is what breaks the cycle the two of them used to form.
 */
 ...
 * alias that names nothing and a repeat of one already used. Shared by
 * `brief.ts` and `normalizeBriefFlow` (`brief-flow.ts`), which narrows
 * further to hunk-only citations after this.
 */
