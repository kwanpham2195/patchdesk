...
const operationSchema = v.strictObject({
  ...
  acknowledgedWarningCodes: v.pipe(v.array(v.pipe(v.string(), v.maxLength(80))), v.maxLength(32)),
  startedAt: v.string(),
  state: v.variant("_tag", [
    v.strictObject({ _tag: v.literal("Requested") }),
    v.strictObject({ _tag: v.literal("OutcomeUnknown") }),
    v.strictObject({ _tag: v.literal("Confirmed"), mergedAt: v.string(), mergeCommitSha: v.optional(v.string()) }),
    v.strictObject({ _tag: v.literal("Rejected"), reason: v.picklist(rejectionReasons) }),
  ]),
});

/** Parses persisted merge-operation evidence without admitting provider payloads or free-form errors. */
export function parseMergeOperation(input: unknown): Result<MergeOperation, InvalidMergeOperation> {
  ...
  if (profileId._tag === "err" || sessionId._tag === "err" || host._tag === "err" || owner._tag === "err" || repo._tag === "err" || number._tag === "err" || expectedHeadSha._tag === "err" || startedAt._tag === "err") return invalid();
  const state = parseState(raw.output.state);
  if (state._tag === "err") return state;
  return ok({ ...raw.output, profileId: profileId.value, sessionId: sessionId.value, pr: { host: host.value, owner: owner.value, repo: repo.value, number: number.value }, expectedHeadSha: expectedHeadSha.value, startedAt: startedAt.value, state: state.value });
}

/** Creates durable merge intent before any remote merge call starts. */
export function requestMergeOperation(input: unknown): Result<MergeOperation, InvalidMergeOperation> {
...
export function markMergeOutcomeUnknown(operation: MergeOperation): Result<MergeOperation, InvalidMergeOperation> {
...
export function confirmMergeOperation(operation: MergeOperation, mergedAt: IsoTimestamp, mergeCommitSha?: GitSha): Result<MergeOperation, InvalidMergeOperation> {
  return operation.state._tag === "OutcomeUnknown"
    ? ok({ ...operation, state: { _tag: "Confirmed", mergedAt, ...(mergeCommitSha === undefined ? {} : { mergeCommitSha }) } })
    : invalid();
}

/** Records a finite merge rejection from before or during the remote operation. */
export function rejectMergeOperation(operation: MergeOperation, reason: string): Result<MergeOperation, InvalidMergeOperation> {
  if ((operation.state._tag !== "Requested" && operation.state._tag !== "OutcomeUnknown") || !rejectionReasons.includes(reason as MergeRejectionReason)) return invalid();
  return ok({ ...operation, state: { _tag: "Rejected", reason: reason as MergeRejectionReason } });
}

function parseState(input: v.InferOutput<typeof operationSchema>["state"]): Result<MergeOperationState, InvalidMergeOperation> {
  if (input._tag !== "Confirmed") return ok(input);
  const mergedAt = parseIsoTimestamp(input.mergedAt);
  const mergeCommitSha = input.mergeCommitSha === undefined ? undefined : parseGitSha(input.mergeCommitSha);
  if (mergedAt._tag === "err" || (mergeCommitSha !== undefined && mergeCommitSha._tag === "err")) return invalid();
  return ok({ _tag: "Confirmed", mergedAt: mergedAt.value, ...(mergeCommitSha === undefined ? {} : { mergeCommitSha: mergeCommitSha.value }) });
}
