...
export function parseUnifiedPatch(patch: string): ReadonlyArray<ParsedPatchFile> {
  const files: Array<{ oldPath: string; newPath: string; kind: ParsedPatchFile["kind"]; oldLines: Set<number>; newLines: Set<number> }> = [];
  let current: (typeof files)[number] | undefined; let oldLine = 0; let newLine = 0; let inHunk = false;
  for (const line of patch.split("\n")) {
    const header = /^diff --git a\/(.+) b\/(.+)$/.exec(line);
    if (header !== null) { const next = { oldPath: header[1] ?? "", newPath: header[2] ?? "", kind: "modified" as const, oldLines: new Set<number>(), newLines: new Set<number>() }; current = next; files.push(next); inHunk = false; continue; }
    ...
    if (line.startsWith("-")) { current.oldLines.add(oldLine); oldLine += 1; continue; }
    if (line.startsWith("+")) { current.newLines.add(newLine); newLine += 1; continue; }
