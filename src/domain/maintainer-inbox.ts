...
export type InboxRecommendedAction =
  ...
  | { readonly kind: "continue_review"; readonly label: "Continue review"; readonly sessionId: ReviewSessionId }
...
export type InboxReviewSummary = {
  ...
  readonly state: "running" | "completed" | "failed" | "draft" | "submitted" | "merged";
...
export function projectMaintainerInboxRow(input: {
  ...
  if (review?.matchesCurrentHead && review.state === "running") categories.push("running");
...
function recommendedActionFor(input: {
  ...
  if (input.review?.matchesCurrentHead && input.review.state === "running") return { kind: "continue_review", label: "Continue review", sessionId: input.review.sessionId };
