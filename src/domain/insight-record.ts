...
export function createInsightRecord(input: { readonly reviewId: ReviewId; readonly type: InsightType; readonly updatedAt: IsoTimestamp }): InsightRecord<unknown> {
  return { schemaVersion: 2, reviewId: input.reviewId, type: input.type, nextToken: 1, updatedAt: input.updatedAt };
}

/** Starts one provider-bound Insight run and captures its immutable selection. */
export function beginInsightRun(record: InsightRecord<unknown>, input: { readonly id: InsightRunId; readonly revision: InsightRevision; readonly provider: InsightProvider; readonly model: string; readonly reasoning: InsightReasoning; readonly startedAt: IsoTimestamp }): Result<InsightRecord<unknown>, "already_running"> {
