...
export function insightOutputGuidance(type: GuidedInsightType): string {
  ...
  return [
    ...
    "A control_flow step is one short pseudocode line, such as on(save), if stops are empty, or return cached result, never a full sentence. A component step is the component name in angle brackets, such as <SessionToolbar>, with hooks as plain names -- only for user-interface component trees such as React or Vue; omit component for code with no user interface.",
