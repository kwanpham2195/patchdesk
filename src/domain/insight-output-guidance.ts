...
export function insightOutputGuidance(type: GuidedInsightType): string {
  ...
  return [
    ...
    "A call_tree step is the real function or method name with its parameter names as written in the patch, such as validateManualDays(command, suggestion), and its children are the calls made inside it; never write a sentence. Name a call once under its caller and never nest a step under itself; when a step has both a definition hunk and a call-site hunk, cite both on that one line. A control_flow step is one short pseudocode line, such as if content is unchanged or return cached result. A component step is the component name in angle brackets, such as <SessionToolbar>, with hooks as plain names.",
