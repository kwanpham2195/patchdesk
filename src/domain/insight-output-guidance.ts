...
export function insightOutputGuidance(type: GuidedInsightType): string {
  ...
  return [
    ...
    "A call_tree is the smallest call graph that makes the change clear: each step is the real function or method name with its parameter names as written in the patch, such as validateManualDays(command, suggestion), never a sentence -- except that a new early return inside an unchanged step is written as return <ErrorName>, with no prose after it, and belongs in control_flow when you can state the condition that reaches it; its children are the calls made inside it, in the order they run; name a call once under each caller and never nest a step under itself; when a step has both a definition hunk and a call-site hunk, cite both on that one line. Never list a test function as a step; tests belong to ownership.notes, not to flow.",
    ...
    "Give an added or removed step the h alias of the hunk that shows it when the patch shows it, and leave citations empty when it does not -- never omit a step for lack of a citation, and never cite a description or commit alias in flow. Mark a step added only when it did not exist before the patch, removed only when it no longer exists after it, and unchanged when it exists on both sides even if its body changed; read the hunk's - and + lines to decide, never its context lines or its @@ header, and never mark a step added because a test for it is new -- a branch a new test reaches was already there; show what changed inside an unchanged step as its added and removed children, and cite on the unchanged step the hunk that changes its body.",
