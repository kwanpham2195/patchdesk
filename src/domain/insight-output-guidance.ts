...
export function insightOutputGuidance(type: GuidedInsightType): string {
  ...
  return [
    ...
    "Give an added or removed step the h alias of the hunk that shows it when the patch shows it, and leave citations empty when it does not -- never omit a step for lack of a citation, and never cite a description or commit alias in flow. An unchanged step carries no citation: if a hunk shows the step, the step is added or removed, not unchanged. A step whose body the patch rewrites is changed, even when its name and call sites are not.",
