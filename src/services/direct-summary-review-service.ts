...
type Gateway = GitHubDirectSummaryGateway &
...
export class DirectSummaryReviewService {
  ...
  async submit(input: {
    ...
    try {
      ...
      if (
        existing?._tag === "OutcomeUnknown" ||
        existing?._tag === "WriteInFlight"
      )
      ...
      if (
        input.event === "APPROVE" &&
        current.value.author !== undefined &&
        current.value.author.toLowerCase() === login.value.toLowerCase()
      )
        return err("self_approval_not_allowed");
      const pending = await this.github.getViewerPendingReview({
        profile: fresh.value.profile,
        pr,
        account: login.value,
      });
      if (pending._tag === "err" || pending.value._tag === "Unavailable")
        return err("unavailable");
      if (pending.value._tag === "Pending") return err("pending_review_exists");
      const baseline = await this.github.getViewerDirectSummaryReviews({
        profile: fresh.value.profile,
        pr,
        account: login.value,
      });
      ...
      if (
        finalPending._tag === "err" ||
        finalPending.value._tag === "Unavailable"
      )
        return err("unavailable");
      if (finalPending.value._tag === "Pending")
        return err("pending_review_exists");
      const operation = {
        requestId: randomUUID(),
        event: input.event,
        bodyDigest: digest(body),
        headSha: fresh.value.session.key.headSha,
        baselineReviewIds: baseline.value.reviews.map(
          (review) => review.reviewId,
        ),
        startedAt: this.now(),
      };
      const inFlight: DirectSummaryReviewState = {
        _tag: "WriteInFlight",
        operation,
      };
      if (!(await this.persist(fresh.value.session, inFlight)))
        return err("outcome_unknown");
      const written = await this.github.createDirectSummaryReview({
        profile: fresh.value.profile,
        pr,
        headSha: fresh.value.session.key.headSha,
        event: input.event,
  ...
  async reconcile(input: {
    ...
    Result<DirectSummaryReviewState | undefined, DirectSummaryReviewFailure>
  > {
    const key = `${input.profileId}:${input.reviewId}`;
    const acquired = this.writeCoordinator.acquire(key);
    if (!acquired) return err("review_write_in_progress");
    try {
      ...
      if (stored === undefined || stored._tag === "Confirmed")
        return ok(stored);
      const account = await this.github.resolveAuthenticatedAccount(
      ...
      const login = parseGitHubLogin(account.value.account);
      if (login._tag === "err") return err("unavailable");
      const read = await this.github.getViewerDirectSummaryReviews({
        profile: current.value.profile,
        pr: sessionPr(current.value.session),
        account: login.value,
      });
      if (read._tag === "err" || !read.value.complete)
        return err("unavailable");
      const baseline = new Set(stored.operation.baselineReviewIds);
      const candidates = read.value.reviews.filter(
        (review) =>
          ...
          submittedDuringOrAfter(
            review.submittedAt,
            stored.operation.startedAt,
          ),
      );
      ...
      if (
        candidates.some(
          (review) =>
            !submittedWithinRecoveryWindow(
              review.submittedAt,
              stored.operation.startedAt,
            ),
        )
      ) {
        const unresolved: DirectSummaryReviewState = {
          _tag: "OutcomeUnknown",
          operation: stored.operation,
          resolution: "manual_resolution_required",
        };
        if (!(await this.save(current.value.session, unresolved)))
          return err("unavailable");
...
export function projectDirectSummaryReview(
  state: DirectSummaryReviewState | undefined,
): DirectSummaryReviewProjection {
  if (state === undefined) return { state: "idle" };
  if (state._tag === "Confirmed")
    return {
      state: "confirmed",
      receipt: { reviewId: state.receipt.reviewId, event: state.receipt.event },
    };
  return {
    state: "recovery_required",
    resolution:
      state._tag === "OutcomeUnknown" ? state.resolution : "check_required",
  };
}

/** GitHub review timestamps have second precision while local intents retain milliseconds. */
function submittedDuringOrAfter(
  submittedAt: IsoTimestamp,
  startedAt: IsoTimestamp,
): boolean {
  return (
    Math.floor(Date.parse(submittedAt) / 1_000) >=
    Math.floor(Date.parse(startedAt) / 1_000)
  );
}
function submittedWithinRecoveryWindow(
  submittedAt: IsoTimestamp,
  startedAt: IsoTimestamp,
): boolean {
