...
export class ReviewObservationService {
  private readonly recovery: ReviewObservationRecovery;

  constructor(private readonly dependencies: ReviewObservationDependencies) {
    this.recovery = new ReviewObservationRecovery(
      this.dependencies,
      (input, review, detectedAt, reason) =>
        this.markUnavailable(input, review, detectedAt, reason),
    );
  }
  ...
  async recoverUnlocked(input: {
    ...
    return this.recovery.recoverUnlocked(input);
  }
  ...
  private async readPolicyEvidence(
    ...
    branch: string,
    branchProtection: Promise<BranchProtectionRead> | undefined,
  ): Promise<Result<GitHubMergePolicyEvidence, GitHubReadFailure> | undefined> {
