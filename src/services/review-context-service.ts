...
export class ReviewContextService {
  ...
  private async loadProjectReviewCriteria(worktreePath: string, rulePaths: ReadonlyArray<string>): Promise<LoadedProjectReviewCriteria> {
    const candidates = [
      { label: "AGENTS.md", path: join(worktreePath, "AGENTS.md"), optional: true },
      { label: "CONTRIBUTING.md", path: join(worktreePath, "CONTRIBUTING.md"), optional: true },
      ...rulePaths.map((path, index) => ({ label: `configured-rule-${index + 1}`, path, optional: false })),
    ];
    ...
    for (const candidate of candidates) {
      const rule = await readRuleFile(candidate.path, candidate.label, totalBytes);
...
async function readRuleFile(path: string, label: string, usedBytes: number): Promise<{ readonly criterion: ProjectReviewCriterion | undefined; readonly missing: boolean }> {
  try {
    const metadata = await lstat(path);
    if (metadata.isSymbolicLink() || !metadata.isFile() || metadata.size > MAX_RULE_BYTES) return { criterion: undefined, missing: false };
    const handle = await open(path, constants.O_RDONLY | constants.O_NOFOLLOW);
    try {
      const openedMetadata = await handle.stat();
      if (!openedMetadata.isFile() || openedMetadata.size > MAX_RULE_BYTES) return { criterion: undefined, missing: false };
      const text = await handle.readFile("utf8");
      const bytes = Buffer.byteLength(text, "utf8");
      if (bytes > MAX_RULE_BYTES || usedBytes + bytes > MAX_TOTAL_RULE_BYTES || containsSensitiveData(text)) return { criterion: undefined, missing: false };
