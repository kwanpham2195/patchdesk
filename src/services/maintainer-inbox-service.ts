...
import type { WorkspaceProfileConfig, WatchedRepoConfig } from "../domain/workspace-profile";
...
export class MaintainerInboxService {
  ...
  async list(profile: WorkspaceProfileConfig): Promise<Result<MaintainerInbox, never>> {
    const authenticated = await this.github.resolveAuthenticatedAccount(profile);
  ...
  private async cachedOrUnavailable(profile: WorkspaceProfileConfig): Promise<Result<MaintainerInbox, never>> {
    const cached = await this.cache.read(profile.id);
    if (cached._tag === "ok") return ok({
      rows: cached.value.rows.map(toCachedRow),
      repositories: profile.repos.map((repo) => ({
        repo,
        state: cached.value.repositories.find((entry) => entry.identity.host === repo.host && entry.identity.owner === repo.owner && entry.identity.repo === repo.repo)?.state ?? "github_auth",
        complete: false,
      })),
...
function compareRows(left: MaintainerInboxRow, right: MaintainerInboxRow): number {
  const priority = (row: MaintainerInboxRow): number => row.categories.includes("saved_review") ? 0 : row.categories.includes("updated_since_review") ? 1 : row.categories.includes("needs_review") ? 2 : row.categories.includes("waiting_for_author") ? 3 : row.categories.includes("checks_failing") ? 4 : row.categories.includes("ready_to_merge") ? 5 : 6;
  return priority(left) - priority(right) || right.updatedAt.localeCompare(left.updatedAt) || left.title.localeCompare(right.title);
}

async function mapConcurrent<T, R>(items: ReadonlyArray<T>, concurrency: number, map: (item: T) => Promise<R>): Promise<ReadonlyArray<R>> {
