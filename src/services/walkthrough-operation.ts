...
const walkthroughSectionSchema = v.strictObject({
  ...
  hunkIds: v.pipe(v.array(v.pipe(v.string(), v.maxLength(MAX_HUNK_ALIAS_LENGTH), v.regex(HUNK_ALIAS))), v.maxLength(MAX_HUNKS_PER_SECTION)),
});
...
export const walkthroughOutputSchema = v.pipe(
  ...
  v.check((output) => output.chapters.reduce((count, chapter) => count + chapter.sections.length, 0) <= MAX_TOTAL_SECTIONS, "Walkthrough output exceeds the aggregate section limit"),
);
...
export function parseWalkthroughOutput(input: unknown): Result<WalkthroughOutput, InvalidWalkthroughOutput> {
...
export async function prepareWalkthroughPrompt(input: WalkthroughInput): Promise<string> {
  ...
  const targetSections = Math.min(12, Math.max(1, Math.ceil(Math.max(1, (patch.match(/^@@ /gm) ?? []).length) / 3)));
  return [
    "Generate a read-only walkthrough for the supplied immutable patch.", insightOutputGuidance("walkthrough"),
    ...
    "HUNK ALIAS MANIFEST:", manifest.value.map((hunk) => `${hunk.id} | ${hunk.path} | ${hunk.header}`).join("\n"),
    "CONTEXT ARTIFACT:", context, "PATCH ARTIFACT:", patch,
  ].join("\n\n");
}

async function readRequiredArtifact(path: string, maxBytes: number): Promise<string> {
  const result = await readBoundedArtifact(path, maxBytes);
  if (result._tag === "ok") return result.value;
  if (result.error.reason === "input_too_large") throw new Error("Walkthrough artifact exceeds the bounded input size");
