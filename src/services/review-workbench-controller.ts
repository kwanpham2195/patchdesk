...
export class ReviewWorkbenchController {
  ...
  async open(input: unknown): Promise<Result<ReviewWorkbenchProjection, ReviewWorkbenchFailure>> {
    ...
    const created = await new ReviewSessionService(this.paths, this.now, dependencies).startReview({ profileId: profileId.value, host: host.value, owner: owner.value, repo: repo.value, number: number.value, headSha: current.value.headSha, ...(current.value.baseSha === undefined ? {} : { baseSha: current.value.baseSha }), isDraft: current.value.isDraft, isOpen: current.value.isOpen, prContext: { title: current.value.title, ...(current.value.description === undefined ? {} : { description: current.value.description }), author: current.value.author, headBranch: current.value.headBranch, baseBranch: current.value.baseBranch }, profile: profile.value, ...(matchingRepo?.localPath === undefined ? {} : { localPath: matchingRepo.localPath }), ...(scope === undefined ? {} : { scope }) });
