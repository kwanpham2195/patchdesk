...
import { buildCodexPrompt, type CodexAppServerClient } from "../adapters/codex/codex-app-server-client";
import type { RepresentedReviewWorktree } from "../domain/represented-review-worktree";
import type { InsightInvocationInput, InsightInvoker } from "./insight-run-coordinator";
import { err } from "../domain/result";

/** Narrow seam for checking the represented worktree's immutable Git head. */
export type WorktreeHeadReader = (worktreePath: string) => Promise<string | undefined>;
...
export class CodexInsightInvoker implements InsightInvoker {
  constructor(
    ...
    private readonly clientFactory: (executablePath: string) => CodexAppServerClient,
  ...
  async invoke(input: InsightInvocationInput, options: { readonly signal: AbortSignal }) {
    if (input.provider !== "codex-cli-account") return err({ reason: "execution_failed" as const });
    const expectedPath = this.paths.worktreeDirectory(input.profileId, input.sessionId);
    const candidatePath = await realpath(input.worktreePath).catch(() => undefined);
    ...
    const result = await this.clientFactory(this.executablePath).run({ worktreePath, expectedHeadSha: head, model: input.model, reasoning: input.reasoning, prompt: prompt.value }, options);
