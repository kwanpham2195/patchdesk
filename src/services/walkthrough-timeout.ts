...
export function walkthroughTimeoutMs(sizes: WalkthroughArtifactSizes): number {
  const patchBytes = Number.isFinite(sizes.patchBytes)
    ? Math.max(0, sizes.patchBytes)
    : 0;
  const contextBytes = Number.isFinite(sizes.contextBytes)
    ? Math.max(0, sizes.contextBytes)
    : 0;
  const hunkCount = Number.isFinite(sizes.hunkCount)
    ? Math.max(0, sizes.hunkCount)
    : 0;
  const sizeSteps = Math.ceil(
    (patchBytes + contextBytes) / WALKTHROUGH_BYTES_PER_STEP,
  );
  const hunkSteps = Math.ceil(hunkCount / WALKTHROUGH_HUNKS_PER_STEP);
  return Math.min(
    WALKTHROUGH_MAX_TIMEOUT_MS,
    WALKTHROUGH_MIN_TIMEOUT_MS +
      Math.max(sizeSteps, hunkSteps) * WALKTHROUGH_TIMEOUT_STEP_MS,
  );
}
...
function countUnifiedDiffHunks(
  patchPath: string,
  signal?: AbortSignal,
): Promise<number> {
