...
Patchdesk runs one exact call-flow analysis in an Electron-as-Node child process. Non-Go sources use the packaged `calldiff` fallback; Go uses Patchdesk's app-owned syntactic rule over the packaged grammar. The parent sends one bounded invocation through stdin. The child returns one strictly parsed and bounded JSON value through stdout, then exits. The child receives the represented-review worktree path and the session's base and head SHAs. It never reads a mutable maintainer checkout or GitHub state.
The strict node contract distinguishes `call`, `branch`, `dependency`, `unresolved`, `reference`, `concurrent`, and `deferred`. Stock CallDiff still emits only `call` and `branch`. Go is an app-owned receiver-collaborator change explanation, not exhaustive syntax coverage: it prefers topmost affected roots in changed files, flattens ordinary control flow, classifies nested calls through receiver-held collaborators as `dependency` inversion-of-control boundaries without claiming an interface or implementation, and omits package/local support calls while retaining receiver method references in their arguments. Normal tree rows have no kind badges; dependency labels alone receive a restrained semantic treatment and one global legend explanation. Technical Raw output retains `[dependency]`, `[unresolved]`, and `[reference]`. An added or removed edge is compared with an existing definition on both revisions; an edge to a one-revision definition stops at that edge.

The first release supports five packaged parsers: Go, JavaScript, JSX, TypeScript, and TSX. Go uses exact `tree-sitter@0.25.1` and `tree-sitter-go@0.25.0` packages. Patchdesk does not download parsers. The macOS package keeps their published N-API prebuilt binaries instead of rebuilding native addons on the packaging machine. Package filters exclude local `build/` outputs so `node-gyp-build` selects the published target prebuild. A target without the exact packaged prebuilt is unsupported until package verification proves it. The child inherits no general process environment and has no GitHub credentials or network capability.
