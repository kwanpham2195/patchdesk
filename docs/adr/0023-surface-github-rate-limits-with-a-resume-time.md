...
> **Superseded in part by ADR 0032, "Refresh the pull requests screen only when
> asked".** The poll scheduler this record describes — its capped backoff ladder,
> and its waiting out a learned resume time — is removed. Rate limits are still
> classified as their own condition, and the resume time is still learned from
> the `rateLimit` field on the listing query and cached per GitHub host, so every
