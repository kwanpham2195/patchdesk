...
Patchdesk owns every Analysis and Walkthrough workflow. The app prepares immutable revision artifacts, starts one finite model run, validates a strict structured result, and decides whether that result may replace retained content or change GitHub review state.
