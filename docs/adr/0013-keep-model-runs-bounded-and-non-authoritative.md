...
Pi-backed Analysis instructions keep trusted Patchdesk policy above untrusted repository criteria and prepared pull request evidence. Pi-backed Analysis receives only session-bound changed-file inspection and immutable Git reads; Pi-backed Walkthrough receives bounded stored artifacts with no tools or write surface. ADR-0016 is a limited exception for either Codex-backed Insight type: Codex may use verified sandboxed read-only inspection tools only against Patchdesk's immutable represented-review worktree, never the maintainer's original checkout. Neither operation can mutate the checkout, write to GitHub, publish feedback, change threads, or merge.
