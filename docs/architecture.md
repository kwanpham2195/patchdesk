...
- `pending-review.ts`, `merge-operation.ts`, and `direct-summary-review.ts` model write intents and their receipts. `local-draft.ts` models a Local draft, the entry `Review.localDrafts` stores for a local Review only. `local-apply-operation.ts` models the Apply suggestion write on a local Review and decides its recovery from file hashes; `local-apply-patch.ts` composes one file's post-image and patch from its current text.
