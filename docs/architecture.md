...
- `review-worktree-service.ts` owns the git commands that create a session checkout: the managed refs, `git worktree add --detach` with the repository's hooks disabled, the ownership marker, and cleanup, for pull request and local sessions alike. An add that fails after Git created the directory removes that worktree, so a retry of the session can succeed.
