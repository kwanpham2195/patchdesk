...
- `pending-review.ts`, `merge-operation.ts`, and `direct-summary-review.ts` model write intents and their receipts. `local-draft.ts` models a Local draft, the entry `Review.localDrafts` stores for a local Review only: a Finding draft, or a maintainer note (ADR 0051). `local-draft-carry.ts` is ADR 0002's carry rule for one draft when the Review moves to a new session: unchanged when its `diff-anchor.ts` fingerprint maps once in the new patch, changed when its surrounding lines are found once in the new file, needs attention otherwise (#452). `local-apply-operation.ts` models the Apply suggestion write on a local Review and decides its recovery from file hashes; `local-apply-patch.ts` composes one file's post-image and patch from its current text.
