...
- The inspector's **Merge** fact uses PR overview's readiness wording: **Ready to merge**, **Blocked · Conflicts +1** (its accessible name lists every cause), **Draft** when the draft is the only cause, **Unknown** while GitHub has not reported mergeability, and **Merged**. It reads the listing's mergeable state, draft state, check rollup, and review decision. The listing does not read branch rules, so a required check still running or a strict "behind base" rule shows only in the Review header, and the fact can read Ready to merge on a row that lacks the Ready to merge category. A cached row shows the cached-data notice instead, because a cache cannot make a current merge-shaped claim.
