...
The Insights line shows one read-only chip per Insight, in the Insights tab order Brief, Walkthrough, and Analysis, reading Failed when that Insight's last run failed, Ready when Patchdesk retains it for the row's current head, Outdated when it is retained for an earlier head, and Not run otherwise. Insights run from the Review workbench, where their results are read; the inspector does not start them.
