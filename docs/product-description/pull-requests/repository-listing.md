...
The Insights line shows one read-only chip per Insight, in the Insights tab order Brief, Walkthrough, and Analysis, each with a status icon: Failed (red circle-x) when that Insight's last run failed, a green check with no word when Patchdesk retains it for the row's current head, Outdated (amber clock) when it is retained for an earlier head, and Not run (dashed circle) otherwise. Insights run from the Review workbench, where their results are read; the inspector does not start them.

Below the Insights line, **Watch** asks Patchdesk to watch an open pull request (ADR 0045); the button then reads **Unwatch**. A watched row carries an eye mark beside its badges, and so does its row in the [Visited pull requests](../foundations/visited-pull-requests.md) column. The inspector hides Watch for a merged pull request. The Review header offers the same toggle on an open Review, and the ⌘K palette offers Watch or Unwatch for a pull request reference typed into it. If the palette resolves a merged or closed pull request, Patchdesk refuses the watch and names that state. In the Review header that refusal appears under the header buttons, and Patchdesk then checks GitHub for the Review: once GitHub confirms the pull request merged or closed, the header turns terminal, reading **Merge · Merged** or **Merge · Closed** with Watch, **Start a review**, and Refresh gone. Watching reads the pull request once from GitHub and changes nothing there. A workspace watches at most 20 pull requests; a 21st Watch is refused with that limit named beside the button.
...
Rows show the title and number, author, labels, Draft or Merged badge inline after the title's last word (Merged carries a merge icon), Brief badge with a green check when a retained Brief exists for the current head, change statistics, CI icon, and relative update age. The CI icon and the inspector's checks fact read **No checks** when GitHub reports no checks on the head, and **Unknown** only when Patchdesk could not read or classify them. The author appears with their GitHub avatar when Patchdesk holds it in the local avatar cache, and with an initials circle otherwise. Missing change statistics show an em dash rather than a fabricated zero, and a zero addition or deletion count reads muted in the row and the inspector instead of green or red. A column with no values on any row of the loaded page, such as Labels on a page with no labelled pull request or Changes when no row has statistics, is hidden, and the space goes to the remaining columns; Author takes most of the Labels width so full logins fit. A row whose head moved since the maintainer last left its Review shows a **New** mark; new comments, reviews, labels, and checks alone do not light it, and a pull request never left in a Review shows none. The inspector adds branch direction, current head, checks, a Merge fact, labels, Insight readiness chips, last-review head, and local Review status.
...
- Merged rows show a Merged badge except under the Merged filter, which already says so; the inspector's status card says Merged, and they have no active-work category.
...
- Ready to merge requires Fresh listing data, a current saved Review, a mergeable state that GitHub's merge state status does not block, and passing checks.
- A ready row shows its Ready to merge evidence among the inspector's facts and opens like any other row.
- The inspector's **Merge** fact uses PR overview's readiness wording: **Ready to merge**, **Blocked · Conflicts +1** (its accessible name lists every cause), **Draft** when the draft is the only cause, **Unknown** while GitHub has not reported mergeability, and **Merged**. It applies the Review's merge readiness rule to GitHub's mergeable state and merge state status, draft state, check rollup, and review decision from the listing, so a branch GitHub's rules block, such as a required check still running or a branch behind its base, reads Blocked in both places. A row with no reported merge state, such as one cached before Patchdesk read it, reads Unknown. A cached row shows the cached-data notice instead, because a cache cannot make a current merge-shaped claim.
