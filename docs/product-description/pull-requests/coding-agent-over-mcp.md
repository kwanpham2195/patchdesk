...
After `refresh_review` prepares a session for new content, the Review stays on the session the maintainer is reading. The header shows **Updates available**, and Apply stays unavailable until the Review moves. The diff never changes under a half-written note.
...
- Drafted from Patchdesk application source at `21457aee`, the tool manifest in `src/mcp/tool-manifest.ts`, the dispatcher and tools in `src/main/mcp/`, and ADR 0052 with its amendments. This page was not re-verified live.
...
- Not observed live: the focused-window silence for `Agent asks for`, and the `<Insight> finished` notification on a local Review (#496); service and notifier tests cover both.
