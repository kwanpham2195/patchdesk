...
The freshness badge shows how long ago the rows were read, such as `GitHub: checked 26m`. A read that is not current names its state before the age: Partial, Cached after refresh failure, Stale, or Unavailable, as in `GitHub: Cached after refresh failure · checked 3h`. While a read runs it says `GitHub: Refreshing`. The badge is also the explicit refresh control and never refreshes itself. There is no separate "Updated … ago" line.
...
> Technical note: The main process measures the same composed search and refuses a request past 256 characters as an invalid request. The renderer check exists so the maintainer sees the reason at the control; a refusal from GitHub would arrive as an unreadable command failure.
...
- Pagination behavior from a later page, page-token invalidation for the new qualifiers, and the reusable-cache exclusion remain unproven by live verification; the 2026-09-14 repository had a single page. The source and tests define those rules, but this pass did not observe them in the running app.
- Live pass on 2026-09-14 against a profile watching one repository with 13 labels and two open pull requests confirmed: the More filters fields and their `login or @me` and `main` placeholders; `No spaces or quotes` under Author for `bad name` with the value kept and no listing change; Escape restoring the last applied Author and keeping the popover open; a 39-character Author with a 100-character Base branch applying normally; and, with five labels ticked, the other eight checkboxes disabled, the button reading `5 labels`, and the `This filter is full` line present.
