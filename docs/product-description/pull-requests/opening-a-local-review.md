...
Every move to a new session carries each Local draft by one rule (ADR 0002, ADR 0050):

- **Unchanged.** The drafted lines and the diff lines around them appear exactly once in the new patch. The draft moves there.
- **Changed since your note.** The drafted lines are gone, but the lines around them are each found exactly once in the new version of the file, read from the new session's own worktree. The draft moves to the lines between them. A draft that changed keeps that label on later Refreshes, because it describes the change since the note was written.
- **Needs attention.** Neither holds: the file is gone, the lines around the draft changed, appear more than once, or have nothing left between them. The draft keeps its original lines and session and stays listed.
...
- An old-side draft (a note on removed lines) is carried only when its fingerprint maps; the changed-since rule reads the new version of the file, which has no removed lines, so such a draft otherwise needs attention.
