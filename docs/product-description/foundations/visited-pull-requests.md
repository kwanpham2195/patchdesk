...
The column has a header strip and a scrolling list. The column sits directly on the window background, like the titlebar, with no border, so the main pane is the one raised surface. The row of the open Review, or the local row of the repository whose local Review is open, has a solid grey fill, a bold title, and a bar at its left edge; a hovered row takes a lighter fill. The strip shows the active workspace's label and the word `Recent`, both in capitals. The list groups rows under `Today`, `Yesterday`, `This week`, and `Earlier`. The groups are local calendar days, not elapsed hours: a pull request opened this morning reads Today whatever the hour, Yesterday is the previous calendar day, This week covers two to six days back, and Earlier holds everything older. A heading appears only above the first row of its group, and a group with no row is not named.

Each row shows the pull request's title, cut off to fit, with the full title as the row's tooltip. When a title is cut off, hovering the row or giving it keyboard focus for about half a second scrolls the title sideways at about 30 pixels a second to its end, holds there, and scrolls back, repeating until the pointer or focus leaves; the title then returns to its start. A title that fits never moves, and with the system's reduced motion setting on, the cut-off title keeps its ellipsis and stays still. The row keeps its height throughout. The line beneath it shows a reference and the age of the last open, labelled `visited`, such as `#119 · visited 5m`. The reference is `#119` when every listed row belongs to one repository, `patchdesk#119` when the rows span repositories under one owner, and `owner/patchdesk#119` when they span owners. The rule looks at the rows on screen, not the watchlist, so a repository the maintainer has stopped watching keeps its rows. The age uses the app-wide short form, such as `now`, `42s`, `5m`, `4h`, or `2d`, then weeks from 28 days; hovering it shows the exact time.

A merged pull request carries `merged, seen 3d` and a closed one carries `closed, seen 12d` at the right of that line, in the same muted grey as the rest of the line, each after a small dot: purple for merged, grey for closed. The two words keep the two ages apart: `visited` is the maintainer's last open, and `seen` is when Patchdesk last saw the state. An open pull request carries no marker. The marker comes from the stored Review, never from a live GitHub read, which is why it is dated.
...
A local row stands for every local Review of one repository checkout, whatever its branch or source, and is named by the repository, such as `kwanpham2195/patchdesk`. A linked worktree of the repository has its own row, named with its folder, such as `kwanpham2195/patchdesk · pd-ux-pass`, and clicking it opens that worktree's working tree. It names no branch, because the checkout can switch branch after the column read its list. Where a pull request row prints its reference, a local row prints `local`, such as `local · visited 5m`. Its age and place in the list come from the most recently opened of its Reviews, and it counts as one row toward the 20. A local row carries no state marker and no eye mark. Local Reviews opened before visits were recorded have no recorded open; a row with none shows no age and is ordered by the last update until the next open. Branch and commit Reviews have no row of their own: the [Local review](../pull-requests/opening-a-local-review.md) picker reopens them.
...
Reading the column, scrolling it, and hovering a title or an age record nothing. Clicking the row of the pull request already on screen, or pressing Enter or Space on it, does nothing: that row is where navigation would land. A selected local row stays live, because the checkout may be on another branch than the local Review on screen. Opening Settings leaves the column as it was beneath the overlay.
...
A local row opens the working tree through the same path as **Open review** in the [Local review](../pull-requests/opening-a-local-review.md) picker. Patchdesk reads the checkout's current branch and working tree, so the click lands on that branch's working-tree Review with its own Local drafts: after a branch switch it opens the new branch's Review, and switching back reopens the first one. Unchanged content lands on the same session and an edit moves the Review to a new one. The titlebar busy bar reads `Opening Review…` while it runs. When navigation is not clear, the click parks the open behind the leave dialog; confirming leaves the Review on screen and then reads the checkout the same way.
...
A failed local open, such as a checkout with no commit or a working tree with merge conflicts, returns to the Pull requests screen and shows `Could not open review` with the local reason.
...
**Supported input and accessibility limits.** Rows and the toggle are keyboard operable. The column carries one Tab stop, which lands on the row of the Review on screen where there is one and on the first row otherwise; Arrow Down and Arrow Up move between rows. The row of the Review on screen, or the local row of its repository, is announced as the current page and stays focusable. Patchdesk does not claim screen-reader, touch, or pen support.

> Technical note: the column reads `GET /v1/sidebar/reviews` for the active profile. The route lists every Review record under the profile, groups local Reviews into one row per repository, orders the rows by last open (falling back to last update for older records), and returns the first 20. A local row carries its host and the ids of its repository's local Reviews, so the click can call `POST /v1/reviews/open-local` with a working-tree source that names no expected branch; that open records the time only when it is a maintainer's open, not the reopen after an Apply. It is not polled; it re-reads on a workspace switch, on every Review open, and when the column mounts.

## Edge cases

- A pull request never opened in Patchdesk does not appear, however recently it changed on GitHub.
- A local row's click on a detached `HEAD` opens the detached working-tree Review. Opening a working tree for the first time on a branch creates that branch's Review, so the row gains a Review without gaining a row.
- A launch restore of a working-tree Review after a branch switch is still refused with the checkout's branch named; [Opening a local Review](../pull-requests/opening-a-local-review.md) owns that refusal.
...
- Live pass on 2026-09-26 over CDP 9233 (#489): after a working-tree Review of the `pd-ux-pass` worktree was opened, the column listed `kwanpham2195/patchdesk · pd-ux-pass` and `kwanpham2195/patchdesk` as two rows; each click opened its own checkout's working tree. Evidence: `/tmp/patchdesk-489/05-sidebar-configured.png`.
