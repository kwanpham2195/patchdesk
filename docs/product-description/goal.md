...
- Workspace settings has no draft: every control saves on its own — text fields on blur and on Enter, selectors and pickers at once — and reports "Saving…", "Saved", or its failure beside itself. Closing Settings asks nothing.
- A workspace save trims its values, never persists a blank list row, and keeps the previously saved value when one is rejected.
...
- A workspace that watches nothing replaces the Pull requests screen with workspace setup: the same Reviewing as and Repositories cards Settings renders, numbered "1. Reviewing as" and "2. Folders and repositories". It never opens Settings, and it reports a missing Git only when the environment read says so.
...
- Settings Review preferences are profile-scoped defaults for the next Analysis run. The screen lists API-key models and reasoning values, reports Codex CLI account availability, and does not overwrite a stored Codex Analysis preference merely by opening.
- An empty watchlist is a successful empty inbox state that makes no repository GitHub read. The first repository enters scope only when the maintainer ticks it, in Settings or in workspace setup on Pull requests.
- A workspace profile owns GitHub host and account, workspace roots, rule paths, watched repositories, and the active-profile choice. It contains no credential. The app calls it a workspace, and its stored identifier is derived from its name on creation and never shown.
...
- Repository discovery scans only saved workspace roots for supported Git origins, excludes already-watched identities from suggestions, preserves their rows in Workspace, and groups unmatched watched entries outside the current roots. A root is saved as soon as its row commits, and is scanned from there.
...
- Pull requests state, rows per page, labels, Awaiting review from you, Review state, and Check status are query state. State is Open or Merged, page size is 10, 25, or 50, labels are bounded to five values of 50 characters, Review state is Any, Not reviewed, Review required, Approved, or Changes requested, and Check status is Any, Pending, Passing, or Failing. Any query change clears pagination. Opaque page tokens are bound to the full query and previous-page history is bounded.
- Review state and Check status are selected in the Pull requests More filters popover. Their active-count badge and individually clearable chips show the requested values. Clear all clears only those two fields in one request, preserving the Selected repository, state, page size, and other filters; profile-scoped choices return after renderer reload.
