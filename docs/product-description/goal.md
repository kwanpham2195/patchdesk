...
- Pull requests state, rows per page, labels, Awaiting review from you, Review state, and Check status are query state. State is Open or Merged, page size is 10, 25, or 50, labels are bounded to five values of 50 characters, Review state is Any, No reviews, Review required, Approved, or Changes requested, and Check status is Any, Pending, Passing, or Failing. Any query change clears pagination. Opaque page tokens are bound to the full query and previous-page history is bounded.
