...
- **Status:** fixed under #348 on branch `fix/insights-header-compact`: on a merged or closed Review, Try again, Re-prepare Review, Run for latest revision, and Start here's Generate walkthrough are not drawn, the same open-only rule as Generate and Regenerate. Start here still offers Open walkthrough when a current Walkthrough exists. Read from source and unit tests; not observed live.
