...
The maintainer checks an unwatched row. The row changes at once; ticks made within a moment of each other are saved together with their identities and local paths, and the workspace reloads once for the batch. `Watch all` and `Watch none` above a folder's list change every row in it as one batch. The repository remains selected in the watchlist on the next scan. Unchecking a watched row removes it without changing the profile's other fields.
...
Checking a candidate begins a watchlist add with its host, owner, repository name, and discovered local path, for the workspace whose repositories the card is showing. Unchecking a watched row begins a watchlist removal. The checkbox state changes optimistically for that row and stays clickable; ticking a row back before its batch is sent cancels that change.

### While the action runs

The scan returns one ready or failed outcome for each saved root. Each root's status is derived from that outcome and the saved watchlist. A successful root with no candidates says `No repositories found in this folder.` A successful root with candidates shows a count such as `2 repositories found · 1 watched` and renders a checkbox for each repository under that root.

Each repository row shows its own pending and error state. One batch is saved at a time; ticks made while one is saving go in the next. A saved batch reports how many repositories were added and removed, then reloads the workspace so the saved profile and grouping become authoritative; the folder is not rescanned. A failed batch returns each of its rows to its confirmed checked state and shows an inline error on each.
...
| Start another action or request a refresh                                                             | Adding a row changes nothing until that row has a value; editing a value changes nothing until the row commits.                                | A second scan can supersede the visible result through reload. Ticks made while a batch is saving are sent after it settles.                               |
