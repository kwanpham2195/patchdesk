...
2. Record the running checkout's `git rev-parse --short HEAD` in `/Users/kwanpham/Work/personal/patchdesk`, then check committed source drift from the linked document's footer pin (`3100615` for untouched documents, `737c515c` for documents revised by the 2026-09-14 UX pass) with `git diff --name-only <pin>..HEAD -- src tests`, working-tree drift with `git diff --name-only -- src tests`, and untracked source paths with `git status --short -- src tests` (also inspect staged source changes with `git diff --cached --name-only -- src tests` if present). Documentation-only commits after the pin do not invalidate these documents. If an application or test path differs, verify from a checkout or worktree at the pin, or record the item as drift/blocked rather than treating the current checkout as evidence for the pinned behavior.
...
## UX pass on 2026-09-14

Four read-only passes and one independent review ran against the maintainer's running `pnpm dev` app on CDP 9233 at `5fe7df3b`, which is the pinned `dd613996` plus renderer failure copy for comment and Finding actions. They covered the Pull requests and cross-cutting pages, the foundations, first-run, and Settings pages, the Review workbench core pages, and the Insights pages. The workspace was the maintainer's Personal profile, watching one repository with 14 listed Visited rows; its represented-review worktrees were missing from disk and no Review had a retained Insight.

The passes read the screen, the accessibility tree, `document.activeElement`, local storage, and local API responses. They made no GitHub write, Insight run, Settings value change, workspace switch, relaunch, or window resize, and pressed neither cleanup action. The review found that the window reported `document.visibilityState` as hidden and ran no animation frames, so any claim that depends on a frame, such as heading focus after a destination change, is inconclusive from these passes.

Results recorded here: the new [Visited pull requests](foundations-and-settings.md#foundationsvisited-pull-requestsmd) checklist has 4 pass, 4 blocked, and 6 rows not run. Dated notes were added to `NAV-01-B`, `APPEAR-01`, `LOG-01`, `LOG-02`, `REPO-01-A`, `DISC-01-A`, `FILTER-09`, `FILTER-10`, and `DIFF-01-C` without changing an ID. Defects confirmed live are `B-09` to `B-13` and `B-18`, with `B-17` and `B-22` confirmed in part, beside the latent `B-14`; `B-15`, `B-16`, `B-19` to `B-21`, `B-23`, and `B-24` are read from source only. All are in [bug-triage.md](../bug-triage.md), and friction that is not a defect is in [ux-friction.md](../ux-friction.md). No document moved to `verified`.

The documents were refreshed against `737c515c` afterwards. `B-09` to `B-12` and all twelve `UX-01` to `UX-12` follow-ups are fixed there, so their rows describe behavior no live pass has seen: `VISITED-12` is back to not run, and the Reviewers, Checks-control, Scope-picker, and Selected-mode rows should be reread against the current text before they are rerun. Nothing here is evidence for a commit later than the pass it records.
