...
Results recorded here: the new [Visited pull requests](foundations-and-settings.md#foundationsvisited-pull-requestsmd) checklist has 4 pass, 4 blocked, and 6 rows not run. Dated notes were added to `NAV-01-B`, `APPEAR-01`, `LOG-01`, `LOG-02`, `REPO-01-A`, `DISC-01-A`, `FILTER-09`, `FILTER-10`, and `DIFF-01-C` without changing an ID. Defects confirmed by the passes and the review are `B-09` to `B-14` in [bug-triage.md](../bug-triage.md), and friction that is not a defect is in [ux-friction.md](../ux-friction.md). No document moved to `verified`.
