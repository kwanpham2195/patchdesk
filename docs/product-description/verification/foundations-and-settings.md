...
| PROFILE-02 | P1 | mouse | Switching profiles applies the selected profile and reloads Pull requests under its identity without reusing the prior workbench. ([Settle](../foundations/workspace-profile-and-identity.md#settle)). | Have two disposable profiles with different labels or watched repositories. | 1. Open the profile switcher.<br>2. Select the second profile.<br>3. Wait for reload. | The active profile label and Pull requests scope change together; the prior Review workbench is not shown under the new profile. | fail — switching from Fixture B to Baseline dirty switch reloaded that profile's two Pull request rows, but the Repository picker stayed at `Select a repository` until `kwanpham2195/patchdesk` was selected manually. Evidence: `/private/tmp/patchdesk-repository-picker-blank-after-switch.png`, `/private/tmp/patchdesk-repository-picker-manually-selected.png`. |
