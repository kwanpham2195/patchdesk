...
A local validation failure keeps the draft. Scalar failures have field-associated guidance; blank-list and duplicate-ID failures use the Profile update failed alert. A request failure also keeps the draft, but the editor shows the request's error message. The maintainer can edit and try again.
...
After a blocked close, profile switch, or New profile request, Cancel leaves the maintainer in Settings with the current draft. Discard resets the draft to the editor's saved baseline, then closes Settings or continues the requested transition. Save continues only when both saving and any required profile selection succeed.
...
- Choosing New profile while dirty uses the same Save, Discard, or Cancel guard as closing Settings and switching profiles.
