...
The Review header shows separate Checks and Merge status controls. A blocked Merge control names the first entry Merge readiness lists, such as "Merge · Blocked · Draft", "Merge · Blocked · Conflicts", "Merge · Blocked · Checks", or "Merge · Blocked · Review"; it reads plain "Blocked" when that entry names no specific cause, such as a generic GitHub block or unknown mergeability. Both open PR overview with Revision, Review status, and Merge readiness expanded. The Checks control also expands the Checks row and moves focus to it; the Merge control leaves Checks collapsed and moves focus to the Merge readiness row. PR overview is a drawer on the right titled "PR overview", with the repository, pull-request number, and title beneath. It has four collapsible rows, in this order:
