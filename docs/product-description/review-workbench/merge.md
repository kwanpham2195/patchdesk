...
The Review header shows separate Checks and Merge status controls. A blocked Merge control names the first entry Merge readiness lists, such as "Merge · Blocked · Draft", "Merge · Blocked · Conflicts", "Merge · Blocked · Checks", or "Merge · Blocked · Review", and counts the other named causes, as in "Merge · Blocked · Draft +1"; its accessible name lists every cause ("merge blocked: draft, conflicts"). It reads plain "Blocked" when no entry names a specific cause, such as a generic GitHub block or unknown mergeability. On a merged or closed Review whose checks are unknown, the Checks control is hidden. Both open PR overview with Revision, Review status, and Merge readiness expanded. The Checks control also expands the Checks row and moves focus to it; the Merge control leaves Checks collapsed and moves focus to the Merge readiness row. PR overview is a drawer on the right titled "PR overview", with the repository, pull-request number, and title beneath. It has four collapsible rows, in this order:

- **Revision.** Its label states freshness: Up to date with GitHub, Newer revision on GitHub, Could not reach GitHub, Not checked with GitHub yet, or Unavailable. Expanded, it shows base ← head branches, "Checked" with a relative time, and the commit and changed-file counts. A merged or closed Review is never refreshed, so it shows neither the freshness label nor "Checked". When GitHub reports a newer head than the represented one, it also shows the Reviewed and Current short SHAs. Without revision data it reads "Revision details unavailable." [Review session and revision](../foundations/review-session-and-revision.md) owns what each freshness state means.
- **Checks.** Its label states the overall check result, such as Passing or Failing, and its body lists the checks. A pull request whose head has no checks reads **No checks** here and on the header control; merge readiness treats it the same as a check list it could not read, so neither blocks a merge on its own.
- **Review status.** One line each for Brief, Analysis, and Walkthrough, with the Insight's icon and its status as an icon and a word: Not run, Running, Outdated, or Failed, or a green check alone for a current result. The Insight tab strip owns those Insights ([Brief](brief.md#arrive)).
- **Merge readiness.** Its label reads Ready to merge, Warnings, Blocked, Draft, or Unknown, and its body holds the reasons, warnings, and the merge command. Draft, in muted text, replaces a red Blocked when the draft state is the only blocker. On a merged or closed Review it reads Merged or Closed, and its body says only "Merge readiness no longer applies."; no blocker cards, draft toggle, base-branch change, or merge command appear.

Readiness distinguishes Ready, Needs acknowledgement, and Blocked. Blockers and warnings remain separate. With none of either, the row says "No blockers or warnings." The merge command appears inside Merge readiness only while the Review is not terminal and readiness is not Blocked. A merged or closed Review adds a footer: "This Review is merged." or the same sentence with closed. Open P0 or P1 findings from the current Analysis appear as one card that counts them; its Review findings action closes PR overview and opens the first of those findings on the Insights tab. A draft or closed pull request is a state, not a failed rule, so its card is neutral; conflicts, failing checks, and other confirmed blocks stay destructive. A sole unknown-mergeability condition is not presented with the same destructive certainty as a confirmed policy block: the row label reads Unknown in an informational tone.

Merge readiness lists entries in one order. An outdated head, a closed pull request, and a draft come first, because none can merge whatever GitHub's rules say. GitHub's reasons follow, then Patchdesk's remaining blockers, such as a high-severity Analysis finding. A Patchdesk blocker that restates a GitHub reason, such as conflicts or checks, appears once as the GitHub reason. A draft on a branch that requires review reads "Merge · Blocked · Draft".
