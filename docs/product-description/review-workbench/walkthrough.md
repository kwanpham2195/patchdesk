...
- `j` moves backward and `k` forward, the reverse of the Vim convention. Confirm whether that direction is intended.
- The reading surface ends with "Each section maps to one part of the patch. Use Back to files when you're done." The Walkthrough has no Back to files control. Confirm whether that line is stale.
- The inline-discussion notice does not say which condition failed, so an outdated Walkthrough and an inline conversation that has not finished loading read the same. Confirm whether the maintainer needs that distinction.
- Arrow keys, `j`, and `k` do not check modifier keys. Confirm whether a Command or Control combination with those keys also moves sections.
- A disabled Regenerate on a merged or closed Review shows no reason on screen. Confirm whether that needs an explanation; the rule itself is confirmed.
