...
| Pull request and Review state                          | Retained Analysis can be read for terminal or outdated Reviews. Generation needs an open Review; on a merged or closed Review, a muted line above the reader says so and describes the disabled Generate analysis and Regenerate. Add to review needs an open, Fresh, patch-backed Review and available pending-review state. | Revision or terminal change discovered before the write prevents it. Generated evidence stays bound to the old represented revision. |
