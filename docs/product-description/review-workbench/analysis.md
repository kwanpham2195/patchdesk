...
- Confirm on a disposable pull request that a published suggestion keeps its single-line and multi-line range and that GitHub offers Commit suggestion or Add suggestion to batch after the review is submitted ([ANALYSIS-03-B to ANALYSIS-03-D](../verification/insights-and-cross-cutting.md#review-workbenchanalysismd)).
...
Baseline drafted from Patchdesk application source commit `3100615`; revised and verified against `737c515c`. The Finding suggestion behavior is revised against `5bc8db68`: its Suggested change panel and Add suggestion to review label were seen in the running app on a fixture Analysis, and its GitHub write, verification rules, and submitted-review result are read from source and are not live-verified.
