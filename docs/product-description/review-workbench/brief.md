...
The Provenance card lists the Revision as a short commit hash, when the Brief was Generated as a local date and time, the Provider and model when the run recorded them, and Citations as "all cited hunks found in the diff", "some cited hunks not found in the diff", or "no hunks cited" when no Flow step cites a hunk. A found citation means the hunk exists in the diff; it does not verify the step it supports. The card ends with a Regenerate button.
