...
Shape groups files by directory and collapses a directory after twelve files into a counted remainder. Each file's note is cut to one line until Show full notes wraps every note in full; Shorten notes cuts them again. Evidence uses its shortest meaningful identifier while preserving full paths in titles. Blast radius answers what this PR could affect in files it doesn't change. A summary line gives how many outside files could be affected, split into source and tests, and the main areas they sit in, or says that nothing outside this PR mentions what it changed. Removed names still mentioned elsewhere come first, then changed names mentioned elsewhere, most files first. Each name shows its count as source and tests and its files grouped by folder, source folders before test folders; past five files a "+N more" button shows the rest, and files the search counted past the twenty it keeps read as "+N not listed". Names nothing outside this PR mentions, usually names the PR adds, fold into one line that opens to list them. The footer says the names were found by text search, so a mention is a place to check, not proof of a call. The No matching test row lists changed files that no changed test file matches by name, folder, or mention; no test is run.
