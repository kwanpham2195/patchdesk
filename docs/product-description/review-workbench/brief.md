...
Generate brief, either Regenerate button, Try again on a failed run, and Run for latest revision on an outdated Brief all open the Insight run dialog. The dialog is titled Run Brief, Regenerate Brief, or Run Brief again. It has three controls: Provider, with the choices API key and Codex CLI account; Model, a searchable list of the provider's models; and Reasoning, the effort levels the chosen model supports. Choosing Codex CLI account shows Load Codex models, because Codex models load only after that explicit action; once loaded, Refresh models replaces it. A confirmation line names the provider, model, and reasoning that will receive the prepared pull-request artifacts and states that Patchdesk keeps all validation, Finding, publication, and merge authority. For Codex CLI account it adds that the provider may inspect the represented-review worktree with read-only tools. Saved Brief preferences seed the dialog where available. Start run stays disabled until a model is chosen.
