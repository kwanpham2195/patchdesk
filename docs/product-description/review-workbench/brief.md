...
Above the reader, the shared Insight header says when the Brief was generated, for example "Generated 7 h ago", or "Outdated · generated 7 h ago" for an earlier revision. Unlike the Analysis and Walkthrough headers, it does not repeat the provider and model, because the Provenance card states them. The header also carries its own Regenerate button, so a current Brief shows Regenerate in two places.
...
| Pull request and Review state                          | Brief can be read for represented open or terminal Reviews. Generation requires an open Review, a valid current session, and patch context; on a merged or closed Review, a muted line above the reader says generation needs an open Review; Generate brief and Regenerate are not drawn. | A newer remote revision does not rewrite the artifact; the result remains evidence for the represented revision.      |
...
- A current Brief shows Regenerate both in the header and in the Provenance card. An outdated or failed Brief hides the header button, while the Provenance card's button stays. A merged or closed Review hides both.
...
- Fixed: a merged or closed Review draws neither Generate brief nor Regenerate and keeps the reason line on screen. See [B-11](../bug-triage.md#b-11-generate-and-regenerate-are-disabled-on-a-merged-or-closed-review-with-no-reason).
