...
A retained result is laid out in two columns on a wide window and one column on a narrow one. The main column holds up to three Flow views, one per kind, the grouped Shape tree, and the Blast radius view, whose Surfaces crossed and No matching test rows appear when they found something and otherwise fold into a "Nothing found:" line that names the empty rows and opens them. The side column holds the Start here card, a read-only copy of the Scope card, and the Provenance card.

The Provenance card lists the Revision as a short commit hash, when the Brief was Generated as a local date and time, the Provider and model when the run recorded them, and Citations as "all cited hunks found in the diff", "some citations or file paths could not be matched to the diff", or "no hunks cited" when no Flow step cites a hunk. A found citation means the hunk exists in the diff; it does not verify the step it supports. The card ends with a Regenerate button.

At the right end of the tab strip, a muted meta line says when the Brief was generated, for example "Generated 7h", or "Outdated · generated 7h" for an earlier revision. A result written in a language other than English also names it, for example "Generated 7h · Vietnamese". Unlike the Analysis and Walkthrough meta lines, it does not repeat the provider and model, because the Provenance card states them. The meta line also carries its own Regenerate button, so a current Brief shows Regenerate in two places.

Older retained Briefs can lack Blast radius or Start here because those fields did not exist when the artifact was stored. Patchdesk omits an absent legacy block without inventing data. When Blast radius was attempted but could not answer, the reader says "Blast radius was not counted:" and names the reason.
...
Generate brief, either Regenerate button, Try again on a failed run, and Run for latest revision on an outdated Brief all open the Insight run dialog. The dialog is titled Run Brief, Regenerate Brief, or Run Brief again. It has four controls: Provider, with the choices API key and Codex CLI account; Model, a searchable list of the provider's models; Reasoning, the effort levels the chosen model supports; and Language, English or Vietnamese, beside Reasoning. Language defaults to English and sets the language of everything the model writes. A Vietnamese Brief writes its ownership notes and Start here in Vietnamese and keeps paths, identifiers, and Flow steps as written, because call-tree steps are identifiers and control-flow steps are pseudocode. Labels and buttons stay English. Changing Language does not mark an existing Brief outdated. Choosing Codex CLI account shows Load Codex models, because Codex models load only after that explicit action; once loaded, Refresh models replaces it. Closing the dialog while Codex models are loading cancels that dialog's ownership of the request without reporting Codex as unavailable; reopening can load the models again. Saved Brief preferences seed the dialog where available. Start run stays disabled until a model is chosen.

Generate brief and both Regenerate buttons are disabled unless the Review is open and at least one Insight provider is available. A merged or closed Review draws neither Generate brief nor either Regenerate button; the header's Merge chip already says the Review is merged or closed, so the Insights tab adds no reason line. When no provider is available, the reader says "No model configured. Add a provider API key, then reload." When only the API key provider has no model, it says "No API-key model configured. Open a run and pick Codex CLI account."
...
Flow draws at most three diff-styled views, at most one per kind — call_tree with real function or method signatures like `validateManualDays(command, suggestion)`, control_flow as short pseudocode lines like `on(save)`, component as a UI tree like `<SessionToolbar>`, state as state names with their transitions like `→ failed on timeout` under each state, and contract as an exported name with its old and new signature rows — each marking a step added, removed, or unchanged in a marker-column row with tree guides, so the maintainer can see whether the change added a step, dropped one, or reordered around it. Each step stays on one line; a step too long for the view ends with an ellipsis and shows its full text on hover. Citation chips sit in their own column at the right of each row and appear only on changed steps and on a contract's exported name. A chip shows the hunk alias, such as `h3`, and names the file on hover. Hunk citations are best effort — a changed step with a cited hunk shows a chip that opens the hunk; a changed step the model could not place in the diff is kept, drawn with a muted marker and no chip, and Provenance reads "some citations or file paths could not be matched to the diff". A contract view carries its chips on the exported name at its root; its signature and field rows show no chip and no muted marker, and they count as uncited only when the root cites no hunk. Flow draws no contract view for a test file or a fixture. A tree left with no surviving changed step is dropped, and Flow itself is absent when no view survives. Each view carries a kind badge and its own Copy as diff action, copying that view back out as fenced diff text for pasting elsewhere. The button reads Copied for about 1.5 seconds after the clipboard accepts the text.

Shape groups files by directory and collapses a directory after twelve files into a counted remainder. Each file's note is cut to one line until Show full notes wraps every note in full; Shorten notes cuts them again. Evidence uses its shortest meaningful identifier while preserving full paths in titles. Blast radius answers what the change could affect in files it doesn't touch. It checks every export the patch declares on an added or removed line, every existing export whose body the patch changes, and any other changed name the model lists, up to twenty names. Each line outside the changed files that names one of them is a mention site: the function, method, or class the line sits in (or top level), its line number, and its kind. A line shaped like a call, `new Name`, or `Name.` is a call; an annotation, type argument, `extends`, `implements`, `as`, or `import type` is type-only; any other import is an import; the rest is other. A summary line counts the functions that call a changed name and the type-only mentions, then names the layers they sit in (main, services, renderer, adapters, domain, runtime), or says that nothing outside the changed files mentions a changed name. Removed names still mentioned elsewhere come first, then changed names mentioned elsewhere, most files first. Each name shows its count by kind, with test files counted once as tests, and its files grouped by folder, source folders before test folders; under each file its sites are listed calls first. Past five sites a "+N more" button shows the rest, and sites past the thirty kept per name read as "+N not listed". A Brief stored before mention sites existed shows files instead: counts split into source and tests, a summary of affected files, and "+N more" past five files. Names nothing outside the changed files mentions, usually names the change adds, fold into one line that counts how many are new and how many changed and opens to list them. The footer says the names were found by text search, so a mention is a place to check, not proof of a call. The No matching test row lists changed code files that no changed test file matches by name, folder, or mention; docs and config files are left out, and no test is run.

Start here gives a lead sentence and an ordered list of files, each with an optional reason. When the Brief stands for the reviewed revision, each file the Diff shows is a link that opens the Diff tab on the full Review diff with that file selected, leaving any commit, Scope, or since-review view. The Insights tab returns to the reader last shown for the session, so it lands back on the Brief. A file of an outdated Brief, or one the Diff does not show, stays plain text. Its button reads Open walkthrough when a current Walkthrough exists for this revision and Generate walkthrough otherwise; a merged or closed Review draws only Open walkthrough.
...
| Pull request and Review state                          | Brief can be read for represented open or terminal Reviews. Generation requires an open Review, a valid current session, and patch context; on a merged or closed Review, Generate brief and Regenerate are not drawn. | A newer remote revision does not rewrite the artifact; the result remains evidence for the represented revision.      |
...
| The pull request, represented revision, pending review, permission, or other target changes elsewhere | Updates can make the represented Review non-current while its Brief stays readable as revision-bound evidence. A merge or close removes Generate brief and Regenerate. | Completion is retained only for its original session and cannot silently become the newer revision's Brief.                                             |
...
**Preferences, keyboard commands, and desktop integration.** Saved provider, model, reasoning, and language values seed later Brief runs. No desktop menu shortcut generates Brief.
...
- An older Brief can omit Blast radius and Start here without showing an error.
- A failed Blast radius calculation explains the omission; a legacy absence is silent. A Surfaces crossed or No matching test row that found nothing is different: it folds into the "Nothing found:" line.
- An older Brief does not record whether a name is new, so each of its names reads as changed.
