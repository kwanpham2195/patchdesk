...
A retained result is laid out in two columns on a wide window and one column on a narrow one. The main column holds up to three Flow views, one per kind, the grouped Shape tree, and the Reach rows that found something, followed by a "Nothing found:" line that names the empty rows and opens them. The side column holds the Start here card, a read-only copy of the Scope card, and the Provenance card.
...
- A failed Reach calculation explains the omission; a legacy absence is silent. A Reach row that found nothing is different: it folds into the "Nothing found:" line.
