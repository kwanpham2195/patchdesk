...
Shape groups files by directory and collapses a directory after twelve files into a counted remainder. Each file's note is cut to one line until Show full notes wraps every note in full; Shorten notes cuts them again. Evidence uses its shortest meaningful identifier while preserving full paths in titles. Blast radius answers what this PR could affect in files it doesn't change. It checks every export the patch declares on an added or removed line, every existing export whose body the patch changes, and any other changed name the model lists, up to twenty names. Each line outside the PR that names one of them is a mention site: the function, method, or class the line sits in (or top level), its line number, and its kind. A line shaped like a call, `new Name`, or `Name.` is a call; an annotation, type argument, `extends`, `implements`, `as`, or `import type` is type-only; any other import is an import; the rest is other. A summary line counts the functions that call something this PR changed and the type-only mentions, then names the layers they sit in (main, services, renderer, adapters, domain, runtime), or says that nothing outside this PR mentions what it changed. Removed names still mentioned elsewhere come first, then changed names mentioned elsewhere, most files first. Each name shows its count by kind, with test files counted once as tests, and its files grouped by folder, source folders before test folders; under each file its sites are listed calls first. Past five sites a "+N more" button shows the rest, and sites past the thirty kept per name read as "+N not listed". A Brief stored before mention sites existed shows files instead: counts split into source and tests, a summary of affected files, and "+N more" past five files. Names nothing outside this PR mentions, usually names the PR adds, fold into one line that counts how many are new and how many changed and opens to list them. The footer says the names were found by text search, so a mention is a place to check, not proof of a call. The No matching test row lists changed code files that no changed test file matches by name, folder, or mention; docs and config files are left out, and no test is run.
