...
import { createReviewSessionId, parseGitSha, parseGitHubHost, parseGitHubOwner, parseGitHubRepoName, parseIsoTimestamp, parsePullRequestNumber, parseWorkspaceProfileId } from "../../src/domain/ids";
...
const review = { ...createReview({ identity, currentSessionId: sessionId, headSha, createdAt: at }), representedRemote: { headSha, pullRequestUpdatedAt: at, snapshotHash: "a".repeat(64) as never, refreshedAt: at } };
const snapshot = { schemaVersion: 1 as const, pullRequest: { ref: { host: identity.host, owner: identity.owner, repo: identity.repo, number: identity.prNumber }, headSha, isDraft: false, isOpen: true, title: "Fixture", author: "fixture", headBranch: "main", baseBranch: "sit", reviewState: "none" as const, mergeability: "mergeable" as const, labels: [], updatedAt: at }, comments: { threads: [], complete: true }, commits: [{ sha: commitSha, message: "Commit", author: "Author", authoredAt: at, isHead: false }], checks: { overall: "passing" as const, checks: [] } };
const session = { id: sessionId, key: { ...identity, headSha }, worktree: { path: "/tmp/patchdesk-worktree" as never, headSha }, pr: { headSha, isDraft: false, isOpen: true } };
...
describe("ReviewCommitService", () => {
  ...
  it("rejects a commit that is not in the represented commit list", async () => {
    const service = new ReviewCommitService(
      ...
      { async load() { return ok(snapshot as never); } },
      { async load() { return ok(session as never); } },
      { async run() { throw new Error("git must not run"); } },
    );
    await expect(service.diff({ profileId, reviewId: review.id, commitSha: headSha })).resolves.toEqual({ _tag: "err", error: { reason: "foreign_commit" } });
  });
});
