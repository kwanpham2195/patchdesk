...
import type {
  ReviewAttempt,
  ReviewAttemptState,
} from "../../src/domain/review-attempt";
import { parseWorkspaceProfileConfig } from "../../src/domain/workspace-profile";
import {
  ReviewWorkbenchProjectionService,
  type ReviewWorkbenchProjection,
} from "../../src/services/review-workbench-projection";
...
const staleHeadSha = must(
  parseGitSha("3333333333333333333333333333333333333333"),
);
  ...
  return {
    async getPullRequest() {
      ...
      return options.current === undefined
        ? missing()
        : { _tag: "ok", value: options.current };
    },
    async loadConversation() {
      calls.push("conversation");
      if (options.comments === undefined) return missing();
      const inline = {
        threads: options.comments.threads.filter(
          (t) => t.location !== undefined,
        ),
        ...(options.comments.complete === undefined
          ? {}
          : { complete: options.comments.complete }),
      };
      const conversation: Conversation = {
        prDescription: "",
        entries: options.comments.threads
          .filter((t) => t.location === undefined)
          .map((t) => ({ _tag: "GeneralThread" as const, thread: t })),
        inline,
        ...(options.comments.complete === undefined
          ? {}
          : { complete: options.comments.complete }),
    ...
    async getPullRequestChecks() {
      calls.push("checks");
      return options.checks === undefined
        ? missing()
        : { _tag: "ok", value: options.checks };
    },
    calls,
  };
}
...
function completedSession(
  ...
  const scope =
    options.incremental === true
      ? {
          ...
          baseSessionId:
            "github.com__centraldigital__patchdesk__pr-42__sha-11111111__000000000000" as never,
          baseHeadSha: must(
            parseGitSha("1111111111111111111111111111111111111111"),
          ),
          headSha,
          comparisonPatchPath: must(
            parseAbsolutePath(paths.comparisonPatchFile(profileId, id)),
          ),
          comparisonMetadataPath: must(
            parseAbsolutePath(paths.comparisonMetadataFile(profileId, id)),
          ),
          previousFindingsPath: must(
            parseAbsolutePath(paths.previousFindingsFile(profileId, id)),
          ),
          lifecyclePath: must(
            parseAbsolutePath(paths.findingLifecycleFile(profileId, id)),
          ),
        }
      : undefined;
  const session = createReviewSession({
    ...
    worktree: {
      path: must(parseAbsolutePath(paths.worktreeDirectory(profileId, id))),
      headSha,
    },
    createdAt: now,
  });
...
function preparedSession(
  ...
  return {
    ...
    ...(currentAttemptId === undefined
      ? {}
      : { currentAttemptId: currentAttemptId as never }),
  };
}
...
function attemptFor(
  ...
  return {
    ...
    reasoning: "medium",
    reviewSkillVersion:
      "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" as never,
    contextHash:
      "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb" as never,
    contextPath: must(
      parseAbsolutePath(paths.attemptContextFile(profileId, session.id, id)),
    ),
    reviewInputPath: must(
      parseAbsolutePath(
        paths.attemptReviewInputFile(profileId, session.id, id),
      ),
    ),
    debugPath: must(
      parseAbsolutePath(paths.attemptDebugFile(profileId, session.id, id)),
    ),
    startedAt: now,
  };
}
...
describe("ReviewWorkbenchProjectionService", () => {
  ...
  it("opens saved local work without reading GitHub", async () => {
    ...
    try {
      ...
      const loaded = await projection.loadLocal({
        profileId,
        sessionId: session.id,
      });
  ...
  it("loads only the Review-represented snapshot without reading GitHub", async () => {
    const github = fakeGitHub({
      current: { ...summary(headSha), title: "Live title" },
      comments: {
        threads: [{ id: "live-thread" as never, state: "open", comments: [] }],
      },
    ...
    try {
      ...
      const loaded = await projection.loadRepresented({
        ...
        freshness: {
          ...
          identity: {
            headSha,
            baseSha: "2".repeat(40) as never,
            canonicalPatchHash: "a".repeat(64) as never,
          },
        },
      });
      expect(loaded).toMatchObject({
        ...
        value: {
          ...
          revision: { freshness: "updates_available", refreshedAt: now },
        },
      });
      expect(github.calls).toEqual([]);
    } finally {
      ...
      const represented = {
        ...
        mergeEvidence: {
          ...
          policy: {
            ...
            appliedRuleset: {
              state: "available" as const,
              value: { rules: [{ type: "required_status_checks" }] },
            },
          },
        },
      };
  ...
  it("refreshes GitHub context without replacing the saved local batch", async () => {
    ...
    try {
      ...
      const refreshed = await projection.refreshRemote({
        profileId,
        sessionId: session.id,
      });
      ...
      expect(refreshed).toMatchObject({
        _tag: "ok",
        value: { freshness: "fresh", checks: { overall: "passing" } },
      });
      expect(github.calls).toEqual(
        expect.arrayContaining(["pull_request", "conversation", "checks"]),
      );
      expect(after).toMatchObject({
        _tag: "ok",
        value: { batchContent: before.value.batchContent },
      });
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  ...
  it("keeps a completed review readable when its local patch is absent, without path fields", async () => {
    ...
    try {
      ...
      expect((await sessions.save(session))._tag).toBe("ok");
      const loaded = await projection.load({
        profileId,
        sessionId: session.id,
      });
  ...
  it("degrades to safe bounded values when GitHub is unavailable", async () => {
    ...
    try {
      ...
      expect(loaded.value.pullRequest).toMatchObject({
        title: "Stored review title",
        reviewState: "unknown",
      });
  ...
  it("marks a changed current head stale while keeping saved evidence readable", async () => {
    ...
    try {
      ...
      expect(loaded.value.insights.analysis.retained?.value).toMatchObject({
        summary: "Persisted review result",
      });
    } finally {
      await rm(root, { recursive: true, force: true });
  ...
  it("projects a retained typed InsightStore result instead of session legacy evidence", async () => {
    ...
    try {
      ...
      const patchHash = must(
        parseContentHash(createHash("sha256").update(patch).digest("hex")),
      );
      const runId = must(
        parseInsightRunId(
          "insight-analysis-1-aaaaaaaaaaaa-github.com__centraldigital__patchdesk__pr-42__review-0c8c9a759258",
        ),
      );
      expect(
        (
          await new InsightStore(paths).save(profileId, {
            schemaVersion: 2,
        ...
        value: {
          insights: {
            analysis: {
              ...
              retained: {
                value: { summary: "Durable analysis" },
                scope: { fileCount: 1 },
              },
            },
          },
      ...
      const corrupted = await projection.loadLocal({
        profileId,
        sessionId: session.id,
      });
        ...
        value: {
          insights: {
            analysis: {
              ...
              retained: { value: { summary: "Durable analysis" } },
            },
          },
        },
      });
      if (corrupted._tag === "ok")
        expect(
          corrupted.value.insights.analysis.retained?.scope,
        ).toBeUndefined();
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  ...
  it("projects persisted first-run Insight run state from storage", async () => {
    ...
    try {
      ...
      expect(
        await projection.loadLocal({ profileId, sessionId: session.id }),
      ).toMatchObject({
        _tag: "ok",
        value: {
          insights: { analysis: { status: "running", activeRun: { runId } } },
        },
      });
  ...
  it("loads an outdated Walkthrough from its retained Session patch instead of the current patch", async () => {
    ...
    try {
      ...
      const oldId = createReviewSessionId({
        profileId,
        host,
        owner,
        repo,
      ...
      const oldSession = createReviewSession({
        ...
        worktree: {
          path: must(
            parseAbsolutePath(paths.worktreeDirectory(profileId, oldId)),
          ),
          headSha: oldHead,
        },
        createdAt: now,
      ...
      const oldHash = must(
        parseContentHash(createHash("sha256").update(oldPatch).digest("hex")),
      );
      const runId = must(
        parseInsightRunId(
          "insight-walkthrough-1-aaaaaaaaaaaa-github.com__centraldigital__patchdesk__pr-42__review-0c8c9a759258",
        ),
      );
      const raw = {
        citationVersion: 2,
        ...
        chapters: [
          {
            ...
            sections: [
              {
                title: "Old section",
                prose: "Old evidence in src/old.ts",
                hunkIds: ["h1"],
              },
            ],
          },
      ...
      const normalized = normalizeNarrativeWalkthrough(raw, oldPatch, {
        profileId,
        sessionId: oldSession.id,
        headSha: oldHead,
        patchHash: oldHash,
      ...
      if (normalized._tag === "err")
        throw new Error(`normalize failed: ${normalized.error.reason}`);
      expect(
        (
          await new InsightStore(paths).save(profileId, {
            schemaVersion: 2,
          ...
          insights: {
            walkthrough: {
              ...
              retained: {
                value: {
                  title: "Retained old walkthrough",
                  snapshot: { sessionId: oldSession.id },
                  chapters: [{ sections: [{ hunks: [{ id: "h1" }] }] }],
                },
              },
            },
          },
      ...
      await writeFile(
        oldSession.patchPath,
        oldPatch.replace("+old-new", "+tampered"),
        "utf8",
      );
      const corrupted = await projection.loadLocal({
        profileId,
          ...
          insights: {
            walkthrough: {
              ...
              retained: {
                value: {
                  title: "Retained old walkthrough",
                  chapters: [{ sections: [{ hunks: [], hunkIds: [] }] }],
                  support: { hunks: [], hunkIds: [] },
                },
              },
            },
          },
      ...
      const unreadable = await projection.loadLocal({
        profileId,
        sessionId: current.id,
      });
      ...
      const missingPatch = await projection.loadLocal({
        profileId,
        sessionId: current.id,
      });
      expect(missingPatch).toMatchObject({
        _tag: "ok",
        value: {
          insights: {
            walkthrough: {
      ...
      const missingSession = await projection.loadLocal({
        profileId,
        sessionId: current.id,
      });
      ...
      expect(
        (
          await new InsightStore(paths).save(profileId, {
            ...
            dismissals: [
              { findingId, reason: "Not applicable.", dismissedAt: now },
            ],
            updatedAt: now,
          })
        )._tag,
        ...
        value: {
          insights: {
            analysis: {
              retained: {
                value: {
                  findings: [{ id: findingId, disposition: "dismissed" }],
                },
              },
            },
          },
  ...
  it("reports a missing incremental comparison truthfully", async () => {
    ...
    try {
      ...
      expect(loaded.value.conversation).toMatchObject({
        entries: [],
        inline: { threads: [] },
      });
    } finally {
  ...
  it("projects a prepared session without attempt history or preparation work", async () => {
    const github = fakeGitHub({
      ...
      comments: {
        threads: [
          { id: "thread-1", body: "Existing discussion", state: "open" },
        ],
      } as never,
    ...
    try {
      ...
      expect(loaded.value.conversation).toMatchObject({
        entries: [{ _tag: "GeneralThread", thread: { id: "thread-1" } }],
      });
      expect(loaded.value.draft).toMatchObject({
        sessionId: session.id,
        items: [],
      });
      expect(loaded.value.mergeReadiness).toEqual({
      ...
      expect(loaded.value.recoveryView).toEqual({
        noticeKey: "ready_to_review",
        tone: "positive",
        actionKey: "run_review",
      });
  ...
  it("projects the analysis matrix into one Review envelope", async () => {
    const cases: ReadonlyArray<{
    ...
    }> = [
      {
        name: "no analysis",
        state: { _tag: "Created" },
        expected: "not_generated",
      },
      {
        name: "running",
        state: { _tag: "Running", attemptId: "001" as never },
        expected: "running",
      },
      {
        name: "current",
      ...
      },
      {
        ...
        state: {
          ...
          error: { category: "flue", message: "safe" },
        },
        expected: "failed",
        visibleResult: true,
      },
    ...
    for (const fixture of cases) {
      const github = fakeGitHub({
        current: fixture.current ?? summary(headSha),
        checks: { overall: "passing", checks: [] },
      });
      const { root, paths, projection, sessions } = await setup(github);
      try {
        ...
        expect((await sessions.save(session))._tag).toBe("ok");
        if (
          fixture.state._tag === "Running" ||
          fixture.state._tag === "ReviewCompleted" ||
          fixture.state._tag === "ReviewFailed"
        ) {
          expect(
            (
              await sessions.saveAttempt(
                profileId,
                session.id,
                attemptFor(
                  paths,
        ...
        expect(loaded._tag).toBe("ok");
        if (loaded._tag === "ok")
          expect(loaded.value.insights.analysis.status, fixture.name).toBe(
            fixture.expected,
          );
      } finally {
        await rm(root, { recursive: true, force: true });
  ...
  it("projects terminal status from the stable Review aggregate", async () => {
    ...
    try {
      ...
      const stableReview: Review = createReview({
  ...
  it("projects a terminal Review status without exposing session internals", async () => {
    ...
    try {
      ...
      expect(loaded).toMatchObject({
        _tag: "ok",
        value: { state: "review", review: { status: "merged" } },
      });
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  ...
  it("projects every recovery matrix action through the workbench boundary", async () => {
    const cases: ReadonlyArray<{
      ...
      },
      {
        name: "interrupted",
        state: { _tag: "Running", attemptId: "001" as never },
        attemptState: { _tag: "Interrupted", interruptedAt: now },
        expected: {
      ...
      },
      {
        ...
        state: {
          _tag: "ReviewFailed",
          attemptId: "001" as never,
          error: { category: "flue", message: "failed" },
        },
        attemptState: {
          _tag: "Failed",
          error: { category: "flue", message: "failed" },
      ...
      {
        name: "unavailable",
        state: { _tag: "Merged", mergedAt: now },
        expected: undefined,
      },
      {
        name: "preparing",
        ...
        expected: { noticeKey: "preparing", tone: "neutral" },
      },
      {
        name: "reconnect",
        state: { _tag: "Running", attemptId: "001" as never },
        attemptState: { _tag: "Running", flueRunId: "flue-1" },
        liveRun: true,
        expected: {
    ...
    for (const fixture of cases) {
      ...
      try {
        const session = preparedSession(
          paths,
          fixture.state,
          fixture.attemptState === undefined ? undefined : "001",
        );
        expect((await sessions.save(session))._tag).toBe("ok");
          ...
          expect(
            (
              await sessions.saveAttempt(
                profileId,
                session.id,
                attemptFor(paths, session, fixture.attemptState),
              )
            )._tag,
          ).toBe("ok");
        }
        const runs = new ReviewRunRegistry(() => now);
        if (fixture.liveRun === true)
          runs.create({ sessionId: session.id, attemptId: "001" });
        const projection = new ReviewWorkbenchProjectionService(
          new ProfileStore(paths),
          sessions,
          github,
          () => now,
          {
            ...
            runs,
            preparation:
              fixture.activePreparation === true
                ? {
                    activeFor: async () => ({
                      _tag: "ok",
                      value: {
        ...
        if (loaded._tag === "ok") {
          if (fixture.expected === undefined)
            expect(loaded.value.recoveryView).toBeUndefined();
      ...
      const projection = new ReviewWorkbenchProjectionService(
        ...
        () => now,
        {
          ...
          preparation: {
            activeFor: async () => ({
              _tag: "err",
              error: { _tag: "PreparationJournalFailed" },
            }),
          },
        },
      );
  ...
  it("rejects unknown profiles and sessions without preparing anything", async () => {
    ...
    try {
      const missing = await projection.load({
        profileId,
        sessionId: sessionId(),
      });
      ...
      const unknownProfile = await projection.load({
        profileId: "unknown" as never,
        sessionId: sessionId(),
      });
      expect(unknownProfile).toEqual({
        _tag: "err",
        error: { _tag: "ProfileNotFound" },
      });
    } finally {
      await rm(root, { recursive: true, force: true });
    }
