...
describe("ReviewInspector", () => {
  it("allows narrow worktree reads and records only bounded telemetry counters", async () => {
    ...
    try {
      ...
      expect(JSON.parse(serializedDebug)).toEqual({ inspectedFileCount: 2, searchCount: 1, gitShowCount: 1, profileRuleLoadFailureCount: 1 });
  ...
  it("keeps inspection results available when debug persistence fails", async () => {
    const root = await mkdtemp(join(tmpdir(), "patchdesk-inspector-debug-failure-"));
    try {
      const inspector = new ReviewInspector({
        worktreePath: root,
      ...
      await expect(inspector.searchFiles("fixture")).resolves.toEqual({ _tag: "ok", value: ["src/a.ts"] });
    } finally { await rm(root, { recursive: true, force: true }); }
  });
