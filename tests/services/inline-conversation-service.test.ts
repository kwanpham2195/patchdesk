...
import { InlineConversationService, type DirectConversationCommand } from "../../src/services/inline-conversation-service";
...
const makeGate = () => ({
  requireFresh: vi.fn(async () => ok({ session: { key: sessionKey }, profile: {} } as never)),
});

const expected = { sessionId: "session-a", headSha, patchHash: "patch-hash" };

function command(overrides: Partial<DirectConversationCommand>): DirectConversationCommand {
  return {
    _tag: "Reply",
    expected,
...
function makeGateway(overrides: Record<string, unknown> = {}) {
  const github = {
    ...
    getPullRequestComments: vi.fn(async () => ok({ threads: [], complete: true })),
    getReviewThreadTarget: vi.fn(async () => ok({ found: true })),
    getReviewCommentTarget: vi.fn(async () => ok({ found: true, viewerDidAuthor: true })),
...
describe("InlineConversationService", () => {
  ...
  it("validates local commands before checking the shared coordinator", async () => {
    ...
    const result = await service.execute({ profileId, reviewId, command: command({ _tag: "Reply", body: "   " }) });
  ...
  it("performs exactly one mutation for a proven Reply", async () => {
    ...
    expect(createThreadReply).toHaveBeenCalledWith(expect.objectContaining({ threadId: "PRRT_thread", body: "Proven" }));
  });
...
describe("FakeGitHubAdapter ownership parity", () => {
  ...
  it("returns found only for a target scoped to the requested pull request", async () => {
    const adapter = new FakeGitHubAdapter({
      ...
      commentTargets: [{ commentId: "PRRC_comment", viewerDidAuthor: true, pr: fixturePr }],
    });
    await expect(
      adapter.getReviewThreadTarget({ profile: {} as never, pr: fixturePr, threadId }),
    ).resolves.toEqual({ _tag: "ok", value: { found: true } });
    await expect(
      adapter.getReviewThreadTarget({ profile: {} as never, pr: foreignPr, threadId }),
    ).resolves.toEqual({ _tag: "ok", value: { found: false } });
    await expect(
      adapter.getReviewCommentTarget({ profile: {} as never, pr: fixturePr, commentId: "PRRC_comment" }),
    ).resolves.toEqual({ _tag: "ok", value: { found: true, viewerDidAuthor: true } });
    await expect(
      adapter.getReviewCommentTarget({ profile: {} as never, pr: foreignPr, commentId: "PRRC_comment" }),
    ).resolves.toEqual({ _tag: "ok", value: { found: false } });
  });
