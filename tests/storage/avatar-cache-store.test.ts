...
describe("avatar cache store", () => {
  ...
  it("hashes the URL as given, so a re-versioned avatar URL is a distinct cache key", () => {
    const before = hashAvatarUrl("https://avatars.githubusercontent.com/u/1?v=3");
    const after = hashAvatarUrl("https://avatars.githubusercontent.com/u/1?v=4");
    expect(before).not.toEqual(after);
    // Deterministic: hashing the same URL twice yields the same key.
    expect(hashAvatarUrl("https://avatars.githubusercontent.com/u/1?v=4")).toEqual(
      after,
    );
  });
