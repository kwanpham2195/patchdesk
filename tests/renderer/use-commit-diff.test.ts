...
import {
  useCommitDiff,
  type CommitDiffLoader,
} from "../../src/renderer/src/hooks/use-commit-diff";
...
describe("useCommitDiff", () => {
  it("ignores a late response from a previous commit selection", async () => {
    ...
    const load: CommitDiffLoader = (sha) =>
      sha === "a".repeat(40) ? first.promise : second.promise;
  ...
  it("fails a response whose commit identity does not match the selected SHA", async () => {
    ...
    await waitFor(() =>
      expect(result.current).toEqual({ _tag: "Failed", sha: "a".repeat(40) }),
    );
  });
