...
describe("dashboard renderer", () => {
  it("uses the current Review and Insight catalog requests", async () => {
    const calls: string[] = [];
    Object.defineProperty(window, "patchdesk", { value: {
      request: async (input: { path?: string }) => {
        calls.push(input.path ?? "");
        if (input.path === "/v1/profiles") return { ok: true, status: 200, body: [{ id: "cfw", label: "CFW", githubHost: "github.com", ghAccount: "fixture" }], correlationId: "x" };
        if (input.path === "/v1/dashboard") return { ok: true, status: 200, body: { rows: [], repos: [] }, correlationId: "x" };
        if (input.path === "/v1/insight-providers") return { ok: true, status: 200, body: { providers: [] }, correlationId: "x" };
