...
test("diff workbench", async ({ page }) => {
  ...
  try {
    ...
    await expect(page.getByRole("region", { name: "Diff workbench" })).toBeVisible();
    await page.getByLabel("Search changed files").fill("b.ts");
    await page.getByRole("button", { name: "Go to finding" }).click();
    await expect(page.getByText("Selected file: src/b.ts")).toBeVisible();
    await page.screenshot({ path: "test-results/milestone-7-browser.png", fullPage: true });
