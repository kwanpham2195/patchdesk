...
it("parses the generated isolated Pi catalog and exposes only its safe projection", async () => {
  ...
  expect(
    PI_AI_CATALOG.find((entry) => entry.provider === "openai")?.models.some(
...
async function fixture(
  ...
  return new LocalPiRuntimeModelCatalog(
    settingsPath,
    new LocalPiProviderCatalog({
...
describe("LocalPiRuntimeModelCatalog", () => {
  it("canonicalizes, orders preferences, deduplicates, and filters configured models", async () => {
    ...
    if (result._tag === "ok") {
      ...
      expect(result.value.models.slice(0, 2).map((model) => model.id)).toEqual([
        "openai/gpt-4",
        "openai/gpt-5",
      ]);
      expect(
        result.value.models.some((model) => model.id === "openai/gpt-4-turbo"),
      ).toBe(true);
      expect(
        result.value.models.some(
          (model) =>
            model.id.startsWith("openai-codex/") ||
            model.id.startsWith("custom/") ||
            model.id === "openai/gpt/invalid",
        ),
      ).toBe(false);
    }
  });
  ...
  it("enumerates the catalog when Pi settings are absent", async () => {
    const root = await mkdtemp(join(tmpdir(), "patchdesk-pi-catalog-"));
    roots.push(root);
    const catalog = new LocalPiRuntimeModelCatalog(
      join(root, "missing-settings.json"),
      new LocalPiProviderCatalog({
        environment: (name) =>
          name === "OPENAI_API_KEY" ? "secret" : undefined,
        homeDirectory: root,
        fileExists: async () => false,
      }),
    );
    ...
    const catalog = await fixture(
      {
        enabledModels: [
          "openai-codex/gpt-5",
          "cloudflare-workers-ai/@cf/meta/llama-3",
        ],
      },
      { OPENAI_API_KEY: "secret" },
    );
