  ...
  private respond(message: Record<string, unknown>): void {
    ...
    if (method === "turn/start") {
      ...
      this.write({
        ...
        params: {
          turnId: "turn-fixture",
          delta: JSON.stringify({ title: "Fixture" }),
        },
      });
...
describe("CodexAppServerClient", () => {
  ...
  it("uses reviewer framing and Simplified Technical English for both Insight types", () => {
    for (const insightType of ["analysis", "walkthrough"] as const) {
      const result = buildCodexPrompt({
        insightType,
        reviewInput: "A bounded review input.",
        policy: "Read only.",
      });
      ...
      expect(result.value).toContain("Use an inverted pyramid");
      expect(result.value).toContain("Never invent motivation");
      expect(result.value).toContain("Do not narrate the patch file by file");
    }
  });

  it("lets each Insight choose its Markdown structure and adds visual guidance only to Analysis", () => {
    const analysis = buildCodexPrompt({
      insightType: "analysis",
      reviewInput: "A bounded review input.",
      policy: "Read only.",
    });
    const walkthrough = buildCodexPrompt({
      insightType: "walkthrough",
      reviewInput: "A bounded review input.",
      policy: "Read only.",
    });
    expect(analysis._tag === "ok" ? analysis.value : "").toContain(
      "Choose the smallest Markdown form",
    );
    expect(walkthrough._tag === "ok" ? walkthrough.value : "").toContain(
      "Choose the smallest Markdown form",
    );
    expect(walkthrough._tag === "ok" ? walkthrough.value : "").not.toContain(
